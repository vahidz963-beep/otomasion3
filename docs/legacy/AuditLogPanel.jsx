...
const COPY = {
  fa: {
    title: 'تاریخچه‌ی تغییرات کاربران',
    subtitle: 'چه کسی، چه زمانی، چه تغییری روی نقش/وضعیت کاربران داده',
    colWhen: 'زمان',
    colAction: 'رویداد',
    colFrom: 'از',
    colTo: 'به',
    actions: {
      ...
      deleted: 'حذف شد',
      password_reset_by_admin: 'رمز عبور توسط مدیر تغییر کرد',
    },
  ...
  en: {
    ...
    subtitle: 'Who changed a user\u2019s role or status, and when',
    colWhen: 'When',
    colAction: 'Event',
    colFrom: 'From',
    colTo: 'To',
...
export default function AuditLogPanel({ lang = 'fa' }) {
  ...
  const fmtValue = (row, value) => {
    ...
    if ((row.action === 'activated' || row.action === 'deactivated') && value != null) {
      return value === 'true' ? t.actions.activated : t.actions.deactivated;
    }
    return value || '—';
  };
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  {t.actions[r.action] || r.action} — {names[r.target_user_id] || r.target_user_id}
                </td>
