...
export default function ReferralPanel({ sourceModule, title = 'ارجاعات', relatedOrderId, relatedDocumentId, sourceRecordId, defaultTarget = 'accounting', compact = false }) {
  ...
  const referralSection = <section className={compact ? 'referral-panel compact' : 'referral-panel'}>
    ...
    {loading ? <div className="referral-empty">در حال دریافت...</div> : rows.length === 0 ? <div className="referral-empty">ارجاعی برای نمایش وجود ندارد.</div> : <div className="referral-list">{rows.map((row) => <article key={row.id} className={`referral-item p${row.priority}`}><div><strong>{row.title_fa}</strong><small>{row.referral_number || '—'} · {MODULE_LABELS_FA[row.source_module] || row.source_module} ← {MODULE_LABELS_FA[row.target_module] || row.target_module} · {PRIORITY_LABELS[row.priority] || row.priority}{row.due_date ? ` · موعد ${formatJalaliDate(row.due_date)}` : ''}</small>{row.response_fa && <p>{row.response_fa}</p>}</div><div className="referral-status-box"><span className={`referral-status ${row.status}`}>{STATUS_LABELS[row.status] || row.status}</span><select disabled={busy} value={row.status} onChange={(e) => changeStatus(row, e.target.value)}><option value="open">باز</option><option value="in_progress">در حال انجام</option><option value="answered">پاسخ داده‌شده</option><option value="done">انجام شده</option><option value="cancelled">لغو شده</option></select></div></article>)}</div>}
