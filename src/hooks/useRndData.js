...
export function useRndData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [incomingRes, projectsRes, stagesRes, templatesRes, stepsRes, costsRes, costSummaryRes, testsRes, refsRes, stockRes] = await Promise.all([
      ...
      supabase.from('v_warehouse_current_stock').select('item_id,item_code,item_name_fa,unit,current_qty,unit_price_estimate,available_for_sale_qty').order('item_code', { ascending: true }).limit(500),
    ]);
