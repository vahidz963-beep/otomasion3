...
function KardexSection({stock,selectedItemId,setSelectedItemId,rows}){
  ...
  return <section className="warehouse-card kardex-page-card"><CardTitle icon={ClipboardList} title="کاردکس کامل کالا"/><div className="filters kardex-search"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="جست‌وجوی کد یا نام کالا؛ نتیجه‌ها خودکار نمایش داده می‌شوند..." /></div>{filtered.length>0&&<div className="kardex-search-results">{filtered.slice(0,20).map(i=><button type="button" className={i.item_id===selectedItemId?'active':''} key={i.item_id} onClick={()=>setSelectedItemId(i.item_id)}><b>{i.item_code}</b><span>{i.item_name_fa}</span><em>موجودی: {formatNumber(i.current_qty)} {i.unit}</em></button>)}</div>}{selected&&<div className="selected-kardex-head"><b>{selected.item_code} · {selected.item_name_fa}</b><span>موجودی فعلی: {formatNumber(selected.current_qty)} {selected.unit}</span></div>}{rows.length?<div className="warehouse-table-wrap kardex-table"><table><thead><tr><th>تاریخ</th><th>سند</th><th>نوع رویداد</th><th>وضعیت سند</th><th>مقدار</th><th>مانده بعد از رویداد</th><th>شرح/مرجع</th></tr></thead><tbody>{rows.slice().reverse().map(r=><tr key={r.tx_id}><td>{formatDate(r.created_at)}</td><td dir="ltr">{r.doc_number||'موقت'}</td><td><span className={`doc-type ${r.direction}`}>{r.transaction_type==='reversal'?'برگشت/لغو':r.direction==='out'?'خروج':'ورود'}</span></td><td>{r.document_status||'—'}</td><td>{formatNumber(r.quantity)}</td><td><b>{formatNumber(r.running_balance)}</b></td><td>{r.note||r.reference_type||'—'}{r.order_code?` · سفارش ${r.order_code}`:''}{r.party_name?` · ${r.party_name}`:''}</td></tr>)}</tbody></table></div>:<Empty text={selected?'کاردکس کاملی برای این کالا ثبت نشده است.':'برای شروع، کالا را جست‌وجو کنید.'}/>}</section>
}
