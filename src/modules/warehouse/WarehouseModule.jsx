...
import {
  ...
  archiveWarehouseShipment,
  getWarehouseExchangeRates,
  setWarehouseExchangeRates,
...
import { createWarehouseCategory, updateWarehouseCategory, archiveWarehouseCategory } from '../../lib/warehouseCategoryApi';
...
export default function WarehouseModule() {
  ...
  const [exchangeRates, setExchangeRates] = useState({ usd_toman: 180000, cny_toman: 26000 });

  const data = useWarehouseData();
  useEffect(() => {
    getWarehouseExchangeRates().then((row) => row && setExchangeRates(row)).catch(() => {});
  }, []);
  ...
  async function runAction(fn, ok) {
    ...
    try {
      ...
      setNotice(getFriendlyErrorMessage(e, 'خطا در عملیات انبار'));
    } finally {
      setBusy(false);
    }
  }
  ...
  return <div className="warehouse-page" dir="rtl">
    ...
    {data.error && <div className="warehouse-message error">{getFriendlyErrorMessage(data.error, 'دریافت اطلاعات انبار با خطا روبه‌رو شد.')}<br /><small dir="ltr">{getTechnicalErrorMessage(data.error)}</small></div>}

    {modal?.type === 'item' && <ItemModal categories={data.categories || []} initial={modal.item} busy={busy} onClose={() => setModal(null)} onSubmit={(payload) => runAction(() => modal.item ? updateWarehouseItem(modal.item.item_id, payload) : createWarehouseItem(payload), modal.item ? 'کالا ویرایش شد.' : 'کالا ثبت شد.')} />}
    ...
    {modal?.type === 'shipment' && <ShipmentModal initial={modal.shipment} busy={busy} onClose={() => setModal(null)} onSubmit={(payload) => runAction(() => modal.shipment ? updateWarehouseShipment(modal.shipment.id, payload) : createWarehouseShipment(payload), modal.shipment ? 'اطلاعات ارسال ویرایش شد.' : 'ارسال دستی ثبت شد.')} />}
    ...
    <nav className="warehouse-tabs">{[['overview','نمای کلی'],['stock','موجودی'],['docs','اسناد'],['kardex','کاردکس'],['sync','همگام‌سازی انبارا'],['shipments','لیست ارسال‌شده‌ها'],['refs','ارجاع و اسناد'],['reports','گزارش‌ها'],['settings','تنظیمات']].map(([k,l])=><button key={k} className={tab===k?'active':''} onClick={()=>setTab(k)}>{l}</button>)}</nav>
    ...
    {!data.loading && tab === 'shipments' && <ShipmentsSection shipments={data.shipments || []} onNew={() => setModal({ type: 'shipment' })} onEdit={(shipment) => setModal({ type: 'shipment', shipment })} onArchive={(shipment) => { if (window.confirm('این ارسال از لیست حذف شود؟ سند و کاردکس حذف نمی‌شوند.')) runAction(() => archiveWarehouseShipment(shipment.id), 'ارسال بایگانی شد.'); }} />}
    ...
    {!data.loading && tab === 'reports' && <ReportsSection report={report} setReport={setReport} stock={data.stock} docs={data.documents} onExport={exportWarehouseReport} exchangeRates={exchangeRates} onSaveRates={(next) => runAction(async () => { const saved = await setWarehouseExchangeRates(next); setExchangeRates(saved || next); }, 'نرخ ارز ذخیره شد و ارزش انبار به‌روزرسانی شد.')} />}
    {!data.loading && tab === 'settings' && <SettingsSection categories={data.categories || []} busy={busy} onCreate={(p) => runAction(() => createWarehouseCategory(p), 'دسته‌بندی ثبت شد.')} onUpdate={(id, p) => runAction(() => updateWarehouseCategory(id, p), 'دسته‌بندی ویرایش شد.')} onArchive={(id) => runAction(() => archiveWarehouseCategory(id), 'دسته‌بندی بایگانی شد.')} />}
  </div>;
}
...
function KardexSection({stock,selectedItemId,setSelectedItemId,rows}){
  ...
  useEffect(()=>{ if (filtered.length && (!selectedItemId || !filtered.some(i=>i.item_id===selectedItemId))) setSelectedItemId(filtered[0].item_id); },[filtered,selectedItemId,setSelectedItemId]);
  const selected=stock.find(i=>i.item_id===selectedItemId);
  const visibleRows=useMemo(()=>rows.filter((r)=>!dateFrom||String(r.created_at||'').slice(0,10)>=dateFrom).filter((r)=>!dateTo||String(r.created_at||'').slice(0,10)<=dateTo),[rows,dateFrom,dateTo]);
  return <section className="warehouse-card kardex-page-card"><CardTitle icon={ClipboardList} title="کاردکس کامل کالا"/><div className="filters kardex-search"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="جست‌وجوی کد یا نام کالا؛ نتیجه‌ها خودکار نمایش داده می‌شوند..." /><label>از تاریخ <JalaliDateInput value={dateFrom} onChange={setDateFrom}/></label><label>تا تاریخ <JalaliDateInput value={dateTo} onChange={setDateTo}/></label></div>{filtered.length>0&&<div className="kardex-search-results">{filtered.slice(0,20).map(i=><button type="button" className={i.item_id===selectedItemId?'active':''} key={i.item_id} onClick={()=>setSelectedItemId(i.item_id)}><b>{i.item_code}</b><span>{i.item_name_fa}</span><em>موجودی: {formatNumber(i.current_qty)} {i.unit}</em></button>)}</div>}{selected&&<div className="selected-kardex-head"><b>{selected.item_code} · {selected.item_name_fa}</b><span>موجودی فعلی: {formatNumber(selected.current_qty)} {selected.unit}</span></div>}{visibleRows.length?<div className="warehouse-table-wrap kardex-table"><table><thead><tr><th>تاریخ</th><th>سند</th><th>نوع رویداد</th><th>وضعیت سند</th><th>مقدار</th><th>مانده بعد از رویداد</th><th>شرح/مرجع</th></tr></thead><tbody>{visibleRows.map(r=><tr key={r.tx_id}><td>{formatDate(r.created_at)}</td><td dir="ltr">{r.doc_number||'موقت'}</td><td><span className={`doc-type ${r.direction}`}>{r.transaction_type==='reversal'?'برگشت/لغو':r.direction==='out'?'خروج':'ورود'}</span></td><td>{r.document_status||'—'}</td><td>{formatNumber(r.quantity)}</td><td><b>{formatNumber(r.running_balance)}</b></td><td>{r.note||r.reference_type||'—'}{r.order_code?` · سفارش ${r.order_code}`:''}{r.party_name?` · ${r.party_name}`:''}</td></tr>)}</tbody></table></div>:<Empty text={selected?'کاردکس کاملی برای این کالا ثبت نشده است.':'برای شروع، کالا را جست‌وجو کنید.'}/>}</section>
}
...
function ReportsSection({report,setReport,stock,docs,onExport,exchangeRates,onSaveRates}){
  const [format,setFormat]=useState('excel');
  const [rateForm,setRateForm]=useState({usdToman: exchangeRates?.usd_toman || 180000, cnyToman: exchangeRates?.cny_toman || 26000});
  ...
  const groups=useMemo(()=>[...new Set(stock.map(i=>String(i.item_group || i.category || '').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'fa')), [stock]);
  ...
  const usdRate=Number(exchangeRates?.usd_toman || 180000), cnyRate=Number(exchangeRates?.cny_toman || 26000);
  // Database values are stored/displayed as rial-backed amounts; the UI formatter
  // presents them in toman. Base currencies are frozen at the reference rates.
  const baseToman=value / 10;
  const baseUsd=baseToman / 180000;
  const baseCny=baseToman / 26000;
  const recalculatedTomanFromUsd=baseUsd * usdRate;
  const recalculatedTomanFromCny=baseCny * cnyRate;
  ...
  return <section className="warehouse-card reports-full"><div className="reports-head"><CardTitle icon={BarChart3} title="گزارش‌های انبار"/><div className="report-format"><span>نوع خروجی</span><select value={format} onChange={e=>setFormat(e.target.value)}><option value="excel">Excel</option><option value="pdf">PDF</option><option value="word">Word</option></select></div></div><div className="report-cards"><button className={report==='low'?'active':''} onClick={()=>setReport('low')}><b>کم‌موجودی</b><span>{low.length} کالا زیر نقطه سفارش</span></button><button className={report==='value'?'active':''} onClick={()=>setReport('value')}><b>ارزش موجودی</b><span>{formatMoney(value)}</span></button><button className={report==='docs'?'active':''} onClick={()=>setReport('docs')}><b>اسناد</b><span>{filteredDocs.length} سند ورود/خروج</span></button></div><div className="report-filter-panel">{report==='docs'?<><select value={filters.docType} onChange={e=>setFilters({...filters,docType:e.target.value})}><option value="all">همه نوع سند</option><option value="in">ورود</option><option value="out">خروج</option></select><select value={filters.docStatus} onChange={e=>setFilters({...filters,docStatus:e.target.value})}><option value="all">همه وضعیت‌ها</option><option value="draft">موقت</option><option value="final">نهایی</option><option value="cancelled">لغوشده</option></select><input value={filters.customer} onChange={e=>setFilters({...filters,customer:e.target.value})} placeholder="مشتری/شهر"/><JalaliDateInput value={filters.dateFrom} onChange={v=>setFilters({...filters,dateFrom:v})}/><JalaliDateInput value={filters.dateTo} onChange={v=>setFilters({...filters,dateTo:v})}/></>:<><select value={filters.group} onChange={e=>setFilters({...filters,group:e.target.value})}><option value="all">همه گروه‌ها</option>{groups.map(g=><option key={g}>{g}</option>)}</select><select value={filters.location} onChange={e=>setFilters({...filters,location:e.target.value})}><option value="">همه مکان‌ها</option>{locations.map(l=><option key={l} value={l}>{l}</option>)}</select><label className="check-filter"><input type="checkbox" checked={filters.lowOnly} onChange={e=>setFilters({...filters,lowOnly:e.target.checked})}/><span>فقط کم‌موجود</span></label></>}</div><section className="warehouse-value-panel"><div className="warehouse-value-head"><div><h3>ارزش کل انبار</h3><small>مبنای تبدیل: ارزش ریالی کالاها</small></div><div className="warehouse-rate-form"><label>دلار (تومان)<input type="number" value={rateForm.usdToman} onChange={e=>setRateForm({...rateForm,usdToman:e.target.value})}/></label><label>یوان (تومان)<input type="number" value={rateForm.cnyToman} onChange={e=>setRateForm({...rateForm,cnyToman:e.target.value})}/></label><button onClick={()=>onSaveRates?.(rateForm)}>ثبت نرخ</button></div></div><div className="warehouse-value-cards"><div><span>ارزش تومانی جدید بر مبنای دلار</span><b>{formatMoney(recalculatedTomanFromUsd * 10)}</b><small>پایه ثابت: {new Intl.NumberFormat('fa-IR',{maximumFractionDigits:2}).format(baseUsd)} دلار · نرخ: {formatMoney(usdRate)}</small></div><div><span>ارزش پایه دلاری</span><b>{new Intl.NumberFormat('fa-IR',{maximumFractionDigits:2}).format(baseUsd)} دلار</b><small>محاسبه پایه با نرخ ۱۸۰٬۰۰۰ تومان</small></div><div><span>ارزش پایه یوانی</span><b>{new Intl.NumberFormat('fa-IR',{maximumFractionDigits:2}).format(baseCny)} یوان</b><small>ارزش تومانی مقایسه‌ای با نرخ جدید یوان: {formatMoney(recalculatedTomanFromCny * 10)}</small></div></div></section><div className="report-output clean"><h3>{preview.title}</h3><p>تعداد ردیف گزارش: {formatNumber(preview.rows.length)}</p><div className="warehouse-table-wrap report-preview"><table><thead><tr>{preview.headers.map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{preview.rows.slice(0,30).map((r,i)=><tr key={i}>{r.map((c,j)=><td key={j}>{c}</td>)}</tr>)}</tbody></table></div><button onClick={output}>دریافت خروجی {format==='excel'?'Excel':format==='pdf'?'PDF':'Word'}</button></div></section>
}

function ShipmentsSection({ shipments = [], onNew, onEdit, onArchive }) {
  const [filters, setFilters] = useState({ q: '', status: 'all', from: '', to: '', minValue: '', maxValue: '' });
  const [sort, setSort] = useState({ key: 'shipment_date', dir: 'desc' });
  const filtered = useMemo(() => sortRows(shipments.filter((s) => {
    const text = `${s.shipment_number || ''} ${s.customer_name || ''} ${s.item_summary || ''} ${s.finance_doc_number || ''} ${s.warehouse_doc_number || ''}`.toLowerCase();
    return (!filters.q || text.includes(filters.q.toLowerCase()))
      && (filters.status === 'all' || s.status === filters.status)
      && (!filters.from || s.shipment_date >= filters.from)
      && (!filters.to || s.shipment_date <= filters.to)
      && (!filters.minValue || Number(s.total_value || 0) >= Number(filters.minValue))
      && (!filters.maxValue || Number(s.total_value || 0) <= Number(filters.maxValue));
  }), sort), [shipments, filters, sort]);
  const th = (key, label) => <th><button className="sort-th" onClick={() => setSort(nextSort(sort, key))}>{label}<span>{sort.key === key ? (sort.dir === 'asc' ? '▲' : '▼') : '↕'}</span></button></th>;
  const monthlyTotal = filtered.reduce((sum, s) => sum + Number(s.total_value || 0), 0);
  function exportExcel() {
    const headers = ['شماره ارسال','تاریخ','مشتری','شهر','سند انبار','فاکتور','شرح کالا','تعداد','کارتون','ارزش','وضعیت','حامل','کد رهگیری','تحویل‌گیرنده','یادداشت'];
    const rows = filtered.map(s => [s.shipment_number, formatDate(s.shipment_date), s.customer_name, s.customer_city, s.warehouse_doc_number, s.finance_doc_number, s.item_summary, s.total_quantity, s.carton_count, s.total_value, shipmentStatus(s.status), s.carrier_name, s.tracking_code, s.receiver_name, s.notes]);
    downloadExcelHtml('warehouse-shipments.xls', headers, rows, 'گزارش لیست ارسال‌شده‌ها');
  }
  function printPdf() {
    const bodyRows = filtered.map((s, i) => `<tr><td>${i + 1}</td><td>${safe(s.shipment_number)}</td><td>${formatDate(s.shipment_date)}</td><td>${safe(s.customer_name || '—')}</td><td>${safe(s.item_summary || '—')}</td><td>${formatNumber(s.total_quantity)}</td><td>${formatNumber(s.carton_count)}</td><td>${formatMoney(s.total_value)}</td><td>${shipmentStatus(s.status)}</td></tr>`).join('');
    openPrintable('گزارش لیست ارسال‌شده‌ها', `<h1>گزارش لیست ارسال‌شده‌ها</h1><p>جمع ارزش گزارش: ${formatMoney(monthlyTotal)}</p><table><thead><tr><th>ردیف</th><th>شماره</th><th>تاریخ</th><th>مشتری</th><th>شرح</th><th>تعداد</th><th>کارتون</th><th>ارزش</th><th>وضعیت</th></tr></thead><tbody>${bodyRows}</tbody></table>`);
  }
  return <section className="warehouse-card shipments-card"><div className="section-head"><CardTitle icon={FileText} title="لیست ارسال‌شده‌ها"/><div className="report-buttons"><button onClick={onNew}>＋ ثبت ارسال دستی</button><button onClick={exportExcel}>Excel</button><button onClick={printPdf}>PDF</button></div></div><div className="report-filter-panel shipment-filters"><input value={filters.q} onChange={e=>setFilters({...filters,q:e.target.value})} placeholder="جست‌وجوی مشتری، کالا، فاکتور..."/><select value={filters.status} onChange={e=>setFilters({...filters,status:e.target.value})}><option value="all">همه وضعیت‌ها</option><option value="ready">آماده به ارسال</option><option value="sent">ارسال‌شده</option><option value="cancelled">لغوشده / ارسال‌نشده</option></select><JalaliDateInput value={filters.from} onChange={v=>setFilters({...filters,from:v})}/><JalaliDateInput value={filters.to} onChange={v=>setFilters({...filters,to:v})}/><input type="number" value={filters.minValue} onChange={e=>setFilters({...filters,minValue:e.target.value})} placeholder="حداقل مبلغ"/><input type="number" value={filters.maxValue} onChange={e=>setFilters({...filters,maxValue:e.target.value})} placeholder="حداکثر مبلغ"/></div><div className="shipment-summary"><b>تعداد ردیف: {formatNumber(filtered.length)}</b><b>جمع ارزش: {formatMoney(monthlyTotal)}</b></div>{filtered.length === 0 ? <Empty text="ارسالی ثبت نشده است یا SQL مرحله ۰۴۱/۰۴۳ هنوز اجرا نشده."/> : <div className="warehouse-table-wrap"><table><thead><tr>{th('shipment_number','شماره')}{th('shipment_date','تاریخ')}{th('customer_name','مشتری')}{th('customer_code','کد مشتری')}{th('item_summary','شرح کالا')}{th('total_quantity','تعداد')}{th('carton_count','کارتون')}{th('total_value','ارزش')}{th('status','وضعیت')}<th>اسناد</th><th>عملیات</th></tr></thead><tbody>{filtered.map(s=><tr key={s.id}><td dir="ltr">{s.shipment_number}</td><td>{formatDate(s.shipment_date)}</td><td>{s.customer_name || s.party_name || '—'}<small>{s.customer_city || ''}</small></td><td dir="ltr">{s.customer_code || '—'}</td><td className="shipment-items">{s.item_summary || '—'}</td><td>{formatNumber(s.total_quantity)}</td><td>{formatNumber(s.carton_count)}</td><td>{formatMoney(s.total_value)}</td><td>{shipmentStatus(s.status)}</td><td><small dir="ltr">{s.warehouse_doc_number || '—'}<br/>{s.finance_doc_number || ''}</small></td><td className="shipment-row-actions"><button className="icon-action-btn" title="ویرایش اطلاعات ارسال" aria-label="ویرایش اطلاعات ارسال" onClick={()=>onEdit(s)}><Edit3 size={15}/></button><button className="icon-action-btn danger" title="بایگانی ارسال" aria-label="بایگانی ارسال" onClick={()=>onArchive?.(s)}><Trash2 size={15}/></button></td></tr>)}</tbody></table></div>}</section>;
}

function ShipmentModal({ initial, busy, onClose, onSubmit }) {
  const [form, setForm] = useState({ customer_name: initial?.customer_name || '', customer_city: initial?.customer_city || '', shipment_date: initial?.shipment_date || new Date().toISOString().slice(0,10), item_summary: initial?.item_summary || '', total_quantity: initial?.total_quantity || 0, carton_count: initial?.carton_count || 0, total_value: initial?.total_value || 0, carrier_name: initial?.carrier_name || '', tracking_code: initial?.tracking_code || '', receiver_name: initial?.receiver_name || '', status: normalizeShipmentStatus(initial?.status) || 'ready', notes: initial?.notes || '' });
  return <Modal title={initial?'ویرایش اطلاعات ارسال':'ثبت ارسال دستی'} onClose={onClose}><form onSubmit={e=>{e.preventDefault();onSubmit(form)}}><div className="form-grid"><label><span>مشتری</span><input value={form.customer_name} onChange={e=>setForm({...form,customer_name:e.target.value})}/></label><label><span>شهر</span><input value={form.customer_city} onChange={e=>setForm({...form,customer_city:e.target.value})}/></label><label><span>تاریخ ارسال شمسی</span><JalaliDateInput value={form.shipment_date} onChange={v=>setForm({...form,shipment_date:v})}/></label><label><span>تعداد کل</span><input type="number" value={form.total_quantity} onChange={e=>setForm({...form,total_quantity:e.target.value})}/></label><label><span>تعداد کارتون</span><input type="number" value={form.carton_count} onChange={e=>setForm({...form,carton_count:e.target.value})}/></label><label><span>ارزش ریال</span><input type="number" value={form.total_value} onChange={e=>setForm({...form,total_value:e.target.value})}/></label><label><span>حامل/باربری</span><input value={form.carrier_name} onChange={e=>setForm({...form,carrier_name:e.target.value})}/></label><label><span>کد رهگیری</span><input value={form.tracking_code} onChange={e=>setForm({...form,tracking_code:e.target.value})}/></label><label><span>تحویل‌گیرنده</span><input value={form.receiver_name} onChange={e=>setForm({...form,receiver_name:e.target.value})}/></label><label><span>وضعیت</span><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option value="ready">آماده به ارسال</option><option value="sent">ارسال‌شده</option><option value="cancelled">لغو</option></select></label><label className="full"><span>شرح کالاها</span><textarea value={form.item_summary} onChange={e=>setForm({...form,item_summary:e.target.value})}/></label><label className="full"><span>یادداشت</span><textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/></label></div><div className="modal-actions"><button type="button" onClick={onClose}>انصراف</button><button disabled={busy} type="submit">ذخیره ارسال</button></div></form></Modal>
}
function normalizeShipmentStatus(status){return ({pending_warehouse:'ready',delivered:'sent',returned:'cancelled'}[status]||status)}
function shipmentStatus(status){const st=normalizeShipmentStatus(status);return ({ready:'آماده به ارسال',sent:'ارسال‌شده',cancelled:'لغو'}[st]||st)}

function SettingsSection({ categories = [], busy, onCreate, onUpdate, onArchive }) { const [editing, setEditing] = useState(null); const [form, setForm] = useState({ name_fa: '', name_en: '', code: '', sort_order: 0 }); function save(e) { e.preventDefault(); if (!form.name_fa.trim()) return; const action = editing ? onUpdate(editing.id, form) : onCreate(form); Promise.resolve(action).then(() => { setEditing(null); setForm({ name_fa: '', name_en: '', code: '', sort_order: 0 }); }); } return <section className="warehouse-card"><CardTitle icon={Settings} title="تنظیمات انبار"/><div className="settings-grid"><Info title="سند موقت ورود/خروج" text="ورود و خروج ابتدا در سند موقت ثبت و بعد نهایی می‌شوند."/><Info title="شماره‌گذاری" text="شماره سندهای ورود و خروج به‌صورت خودکار ساخته می‌شود."/><Info title="حذف نرم کالا" text="کالا غیرفعال می‌شود و تاریخچه آن حفظ خواهد شد."/><Info title="اتصال ماژول‌ها" text="سفارش، فاکتور و تولید با گردش انبار هماهنگ هستند."/></div><div className="warehouse-category-settings"><div className="section-head"><CardTitle icon={Package} title="مدیریت دسته‌بندی کالاها"/><span className="note">دسته‌بندی‌های بایگانی‌شده در فهرست کالاهای جدید نمایش داده نمی‌شوند.</span></div><form className="category-form" onSubmit={save}><input value={form.name_fa} onChange={(e)=>setForm({...form,name_fa:e.target.value})} placeholder="نام دسته‌بندی فارسی" required/><input value={form.name_en} onChange={(e)=>setForm({...form,name_en:e.target.value})} placeholder="نام انگلیسی"/><input value={form.code} onChange={(e)=>setForm({...form,code:e.target.value})} placeholder="کد اختیاری"/><input type="number" value={form.sort_order} onChange={(e)=>setForm({...form,sort_order:e.target.value})} placeholder="ترتیب"/><button disabled={busy} type="submit">{editing ? 'ذخیره ویرایش' : '＋ افزودن دسته'}</button>{editing && <button type="button" onClick={()=>{setEditing(null);setForm({name_fa:'',name_en:'',code:'',sort_order:0})}}>انصراف</button>}</form><div className="category-list">{categories.map((c)=><div className="category-row" key={c.id}><b>{c.name_fa}</b><small>{c.code || c.name_en || 'بدون کد'}</small><span><button type="button" onClick={()=>{setEditing(c);setForm({name_fa:c.name_fa||'',name_en:c.name_en||'',code:c.code||'',sort_order:c.sort_order||0})}}>ویرایش</button><button type="button" className="danger" onClick={()=>onArchive(c.id)}>بایگانی</button></span></div>)}</div></div></section>}
function ItemModal({initial,categories=[],busy,onClose,onSubmit}){const [form,setForm]=useState({item_code:initial?.item_code||'',item_name_fa:initial?.item_name_fa||'',item_group:initial?.item_group||initial?.category||'Bobbin',unit:initial?.unit||'عدد',location:initial?.location||'',reorder_point:initial?.reorder_point||initial?.min_stock_threshold||0,unit_price_estimate:initial?.unit_price_estimate||0,stock_quantity:initial?.current_qty ?? 0,current_qty_snapshot:initial?.current_qty ?? 0});return <Modal title={initial?'ویرایش کالا':'کالای جدید'} onClose={onClose}><form onSubmit={e=>{e.preventDefault();onSubmit(form)}}><div className="form-grid"><label><span>کد کالا</span><input disabled={!!initial} value={form.item_code} onChange={e=>setForm({...form,item_code:e.target.value})} required/></label><label><span>نام کالا</span><input value={form.item_name_fa} onChange={e=>setForm({...form,item_name_fa:e.target.value})} required/></label><label><span>گروه</span><select value={form.item_group} onChange={e=>setForm({...form,item_group:e.target.value})}>{(categories.length ? categories.map((c) => c.name_fa) : GROUPS).map(g=><option key={g}>{g}</option>)}</select></label><label><span>واحد</span><input value={form.unit} onChange={e=>setForm({...form,unit:e.target.value})}/></label><label><span>مکان</span><input value={form.location} onChange={e=>setForm({...form,location:e.target.value})}/></label><label><span>تعداد موجودی</span><input type="number" value={form.stock_quantity} onChange={e=>setForm({...form,stock_quantity:e.target.value})}/></label><label><span>نقطه سفارش</span><input type="number" value={form.reorder_point} onChange={e=>setForm({...form,reorder_point:e.target.value})}/></label><label><span>قیمت مرجع ریال</span><input type="number" value={form.unit_price_estimate} onChange={e=>setForm({...form,unit_price_estimate:e.target.value})}/></label></div><div className="note">تغییر تعداد موجودی به صورت ردیف اصلاحی در سند موقت ثبت می‌شود و بعد از تأیید سند نهایی خواهد شد.</div><div className="modal-actions"><button type="button" onClick={onClose}>انصراف</button><button disabled={busy} type="submit">ثبت</button></div></form></Modal>}
