...
export default function WarehouseModule() {
  ...
  return <div className="warehouse-page" dir="rtl">
    ...
    {!data.loading && tab === 'stock' && <StockSection groups={[...new Set(data.stock.map((i) => i.item_group || i.category).filter(Boolean)]).sort((a,b)=>a.localeCompare(b,'fa'))} stock={filteredStock} query={query} setQuery={setQuery} groupFilter={groupFilter} setGroupFilter={setGroupFilter} statusFilter={statusFilter} setStatusFilter={setStatusFilter} selectedItemId={selectedItemId} kardexRows={kardex.rows} busy={busy} onIn={(item)=>setModal({type:'movement',item,direction:'in'})} onOut={(item)=>setModal({type:'movement',item,direction:'out'})} onOpenKardex={(item)=>{setSelectedItemId(item.item_id);setTab('kardex')}} onEdit={(item)=>setModal({type:'item',item})} onStocktake={openStocktake} onDeactivate={(item)=>runAction(()=>deactivateWarehouseItem(item.item_id),'کالا غیرفعال شد.')} exportStock={exportStock} printStock={printStock}/>} 
