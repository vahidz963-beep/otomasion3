...
function ImportModal({busy,onClose,onSubmit}){const [text,setText]=useState('item_code,quantity,unit\nTR-220-12,5,عدد\nPCB-CTRL,10,عدد');return <Modal title="ورود از اکسل/CSV انبارا" onClose={onClose}><div className="note">قالب پیشنهادی ستون‌ها: item_code, item_name_fa, item_group, unit, location, quantity, reorder_point, unit_price_estimate. فعلاً برای نسخه آنلاین، CSV ساده paste می‌شود.</div><textarea className="import-text" value={text} onChange={e=>setText(e.target.value)}/><div className="modal-actions"><button type="button" onClick={()=>downloadCsv('warehouse_import_template.csv',[['item_code','item_name_fa','item_group','unit','location','quantity','reorder_point','unit_price_estimate'],['TR-220-12','ترانس سفارشی','Finished','عدد','A1','5','2','740000000']])}>دانلود قالب CSV</button><button type="button" onClick={onClose}>انصراف</button><button disabled={busy} onClick={()=>onSubmit({fileName:'manual-import.csv',rows:parseCsvText(text),notes:'Manual CSV import'})}>ثبت Snapshot</button></div></Modal>}
