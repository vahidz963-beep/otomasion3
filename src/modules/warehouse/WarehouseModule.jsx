...
export default function WarehouseModule() {
  ...
  return <div className="warehouse-page" dir="rtl">
    ...
    {!data.loading && tab === 'shipments' && <ShipmentsSection shipments={data.shipments || []} onNew={() => setModal({ type: 'shipment' })} onEdit={(shipment) => setModal({ type: 'shipment', shipment })} />}
...
function ShipmentsSection({ shipments = [], onNew, onEdit }) {
  ...
  return <section className="warehouse-card shipments-card"><div className="section-head"><CardTitle icon={FileText} title="لیست ارسال‌شده‌ها"/><div className="report-buttons"><button onClick={onNew}>＋ ثبت ارسال دستی</button><button onClick={exportExcel}>Excel</button><button onClick={printPdf}>PDF</button></div></div><div className="report-filter-panel shipment-filters"><input value={filters.q} onChange={e=>setFilters({...filters,q:e.target.value})} placeholder="جست‌وجوی مشتری، کالا، فاکتور..."/><select value={filters.status} onChange={e=>setFilters({...filters,status:e.target.value})}><option value="all">همه وضعیت‌ها</option><option value="pending_warehouse">در انتظار تکمیل انبار</option><option value="ready">آماده ارسال</option><option value="sent">ارسال‌شده</option><option value="delivered">تحویل‌شده</option><option value="returned">برگشتی</option><option value="cancelled">لغو</option></select><JalaliDateInput value={filters.from} onChange={v=>setFilters({...filters,from:v})}/><JalaliDateInput value={filters.to} onChange={v=>setFilters({...filters,to:v})}/><input type="number" value={filters.minValue} onChange={e=>setFilters({...filters,minValue:e.target.value})} placeholder="حداقل مبلغ"/><input type="number" value={filters.maxValue} onChange={e=>setFilters({...filters,maxValue:e.target.value})} placeholder="حداکثر مبلغ"/></div><div className="shipment-summary"><b>تعداد ردیف: {formatNumber(filtered.length)}</b><b>جمع ارزش: {formatMoney(monthlyTotal)}</b></div>{filtered.length === 0 ? <Empty text="ارسالی ثبت نشده است یا SQL مرحله ۰۴۱ هنوز اجرا نشده."/> : <div className="warehouse-table-wrap"><table><thead><tr>{th('shipment_number','شماره')}{th('shipment_date','تاریخ')}{th('customer_name','مشتری')}{th('item_summary','شرح کالا')}{th('total_quantity','تعداد')}{th('carton_count','کارتون')}{th('total_value','ارزش')}{th('status','وضعیت')}<th>اسناد</th><th>عملیات</th></tr></thead><tbody>{filtered.map(s=><tr key={s.id}><td dir="ltr">{s.shipment_number}</td><td>{formatDate(s.shipment_date)}</td><td>{s.customer_name || s.party_name || '—'}<small>{s.customer_city || ''}</small></td><td className="shipment-items">{s.item_summary || '—'}</td><td>{formatNumber(s.total_quantity)}</td><td>{formatNumber(s.carton_count)}</td><td>{formatMoney(s.total_value)}</td><td>{shipmentStatus(s.status)}</td><td><small dir="ltr">{s.warehouse_doc_number || '—'}<br/>{s.finance_doc_number || ''}</small></td><td><button onClick={()=>onEdit(s)}>ویرایش</button></td></tr>)}</tbody></table></div>}</section>;
}

function ShipmentModal({ initial, busy, onClose, onSubmit }) {
  const [form, setForm] = useState({ customer_name: initial?.customer_name || '', customer_city: initial?.customer_city || '', shipment_date: initial?.shipment_date || new Date().toISOString().slice(0,10), item_summary: initial?.item_summary || '', total_quantity: initial?.total_quantity || 0, carton_count: initial?.carton_count || 0, total_value: initial?.total_value || 0, carrier_name: initial?.carrier_name || '', tracking_code: initial?.tracking_code || '', receiver_name: initial?.receiver_name || '', status: initial?.status || 'pending_warehouse', notes: initial?.notes || '' });
  return <Modal title={initial?'ویرایش اطلاعات ارسال':'ثبت ارسال دستی'} onClose={onClose}><form onSubmit={e=>{e.preventDefault();onSubmit(form)}}><div className="form-grid"><label><span>مشتری</span><input value={form.customer_name} onChange={e=>setForm({...form,customer_name:e.target.value})}/></label><label><span>شهر</span><input value={form.customer_city} onChange={e=>setForm({...form,customer_city:e.target.value})}/></label><label><span>تاریخ ارسال شمسی</span><JalaliDateInput value={form.shipment_date} onChange={v=>setForm({...form,shipment_date:v})}/></label><label><span>تعداد کل</span><input type="number" value={form.total_quantity} onChange={e=>setForm({...form,total_quantity:e.target.value})}/></label><label><span>تعداد کارتون</span><input type="number" value={form.carton_count} onChange={e=>setForm({...form,carton_count:e.target.value})}/></label><label><span>ارزش ریال</span><input type="number" value={form.total_value} onChange={e=>setForm({...form,total_value:e.target.value})}/></label><label><span>حامل/باربری</span><input value={form.carrier_name} onChange={e=>setForm({...form,carrier_name:e.target.value})}/></label><label><span>کد رهگیری</span><input value={form.tracking_code} onChange={e=>setForm({...form,tracking_code:e.target.value})}/></label><label><span>تحویل‌گیرنده</span><input value={form.receiver_name} onChange={e=>setForm({...form,receiver_name:e.target.value})}/></label><label><span>وضعیت</span><select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option value="pending_warehouse">در انتظار تکمیل انبار</option><option value="ready">آماده ارسال</option><option value="sent">ارسال‌شده</option><option value="delivered">تحویل‌شده</option><option value="returned">برگشتی</option><option value="cancelled">لغو</option></select></label><label className="full"><span>شرح کالاها</span><textarea value={form.item_summary} onChange={e=>setForm({...form,item_summary:e.target.value})}/></label><label className="full"><span>یادداشت</span><textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/></label></div><div className="modal-actions"><button type="button" onClick={onClose}>انصراف</button><button disabled={busy} type="submit">ذخیره ارسال</button></div></form></Modal>
}
function shipmentStatus(status){return ({pending_warehouse:'در انتظار تکمیل انبار',ready:'آماده ارسال',sent:'ارسال‌شده',delivered:'تحویل‌شده',returned:'برگشتی',cancelled:'لغو'}[status]||status)}
