...
export default function ProductionModule({ lang = 'fa' }) {
  ...
  return <div className="production-page" dir="rtl" lang={lang}>
    ...
    {modal?.type === 'output' && <OutputModal order={modal.order || selectedOrder} stock={data.stock} busy={busy} onClose={() => setModal(null)} onSubmit={(payload) => runAction(() => registerProductionOutput(payload), 'خروجی تولید به انبار اضافه شد.')} />}
  </div>;
}
...
function PlanningSection({ orders, plans, busy, onPlan, onDeletePlan }) {
  ...
  return <section className="production-card planning-timeline-card"><div className="section-head"><CardTitle icon={BarChart3} title="برنامه‌ریزی تولید" /><div className="filters"><JalaliDateInput value={monthStart} onChange={(v)=>setMonthStart(v || monthStart)} /></div></div><div className="month-timeline"><div className="timeline-days">{days.map((d,i)=><span key={d}>{i+1}<small>{formatDate(d).slice(5)}</small></span>)}</div><div className="timeline-bars">{orders.map((o,idx)=>{const p=plans.find(x=>x.production_order_id===o.id);const s=p?.planned_start||o.planned_start;const e=p?.planned_end||o.planned_end;const left=pos(s);const right=pos(e||s);const width=Math.max(1,right-left+1);return <div key={o.id} className={`timeline-row ${o.status === 'completed' || o.status === 'delivered_to_warehouse' ? 'completed' : ''}`}><label>{o.code}</label><div className="timeline-track"><button className="timeline-bar" style={{insetInlineStart:`${left/30*100}%`,width:`${width/30*100}%`}} onClick={()=>onPlan(o)}>{o.product_name_fa}</button></div></div>})}</div></div>{orders.length===0 ? <Empty/> : <div className="planning-grid">{orders.map((o) => { const p = plans.find((x) => x.production_order_id === o.id); return <article key={o.id} className="planning-card"><header><h3>{o.code}</h3><Status status={o.delivery_status} /></header><p>{o.product_name_fa} · {o.customer_name || '—'}</p><div className="detail-mini-grid"><Info label="شروع" value={formatDate(p?.planned_start || o.planned_start)} /><Info label="پایان" value={formatDate(p?.planned_end || o.planned_end)} /><Info label="روز کاری" value={p?.work_days || o.work_days} /><Info label="نفرساعت" value={p?.total_man_hours || o.total_man_hours} /></div><div className="planning-actions"><button onClick={() => onPlan(o)}>ثبت/ویرایش برنامه</button><button className="danger" disabled={busy || !(p || o.planned_start || o.planned_end)} onClick={() => onDeletePlan(o)}>حذف از برنامه</button></div></article>; })}</div>}</section>;
}
...
function BomModal({ bom, stock, orders = [], existingItems = [], busy, onClose, onSubmit }) {
  const [form, setForm] = useState({ id: bom?.id || null, warehouse_item_id: bom?.warehouse_item_id || '', related_production_order_id: bom?.related_production_order_id || '', related_order_id: bom?.related_order_id || '', product_name_fa: bom?.product_name_fa || '', product_name_en: bom?.product_name_en || '', version_no: bom?.version_no || 'v1', status: bom?.status || 'draft', unit: bom?.unit || 'عدد', notes: bom?.notes || '' });
