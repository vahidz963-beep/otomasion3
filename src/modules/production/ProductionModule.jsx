...
import {
  ...
  saveProductionBom,
  saveProductionPlan,
  publishBomToWarehouse,
  registerProductionOutput,
  deleteProductionPlan,
  prepareProductionMaterialsFromBom,
  issueProductionMaterial,
  issueAllProductionMaterials,
...
export default function ProductionModule({ lang = 'fa' }) {
  ...
  const selectedMaterials = useMemo(() => data.materialUsage.filter((m) => m.production_order_id === selectedProductionId), [data.materialUsage, selectedProductionId]);
  const selectedOutputs = useMemo(() => data.outputs.filter((o) => o.production_order_id === selectedProductionId), [data.outputs, selectedProductionId]);
  ...
  return <div className="production-page" dir="rtl" lang={lang}>
    ...
    <nav className="production-tabs">
      {[
        ['overview', 'نمای کلی'], ['flow', 'مراحل تولید'], ['incoming', 'لیست سفارش‌ها'], ['planning', 'برنامه‌ریزی تولید'], ['bom', 'فرمول تولید کالا'], ['qc', 'کنترل کیفیت'], ['docs', 'اسناد تولید'], ['refs', 'ارجاع و اسناد'], ['settings', 'تنظیم مراحل']
    ...
    {!data.loading && tab === 'refs' && <div className="production-grid"><ReferralPanel sourceModule="production" title="ارجاع و اسناد تولید" defaultTarget="warehouse" /></div>}
    ...
    {modal?.type === 'accept' && <AcceptOrderModal order={modal.order} templates={data.templates} busy={busy} onClose={() => setModal(null)} onSubmit={(payload) => runAction(async () => { const id = await acceptProductionOrder(payload); setSelectedProductionId(id); setTab('planning'); }, 'سفارش وارد تولید شد و به برنامه‌ریزی تولید منتقل شد.')} />}
    ...
    {modal?.type === 'bom' && <BomModal bom={modal.bom} stock={data.stock} orders={data.productionOrders} existingItems={data.bomItems.filter((i) => i.bom_id === modal.bom?.id)} busy={busy} onClose={() => setModal(null)} onSubmit={(payload) => runAction(() => saveProductionBom(payload), 'فرمول تولید ذخیره شد.')} />}
    ...
    {modal?.type === 'output' && <OutputModal order={modal.order || selectedOrder} stock={data.stock} existingOutputs={data.outputs.filter((o) => o.production_order_id === (modal.order?.id || selectedOrder?.id))} busy={busy} onClose={() => setModal(null)} onSubmit={(payload) => runAction(() => registerProductionOutput(payload), 'خروجی‌های تولید به انبار اضافه شد.')} />}
  </div>;
}
...
function FlowSection({ orders, stages, selectedOrder, selectedStages, selectedPlan, selectedQc, selectedDocs, selectedMaterials, query, setQuery, busy, onSelect, onClose, onSetStage, onPlan, onQc, onDocument, onPrepareMaterials, onIssueAllMaterials, onIssueMaterial, onOutput, onExport, onPrint }) {
  return <section className="production-card production-flow-workspace">
    ...
    {selectedOrder && <div className="production-drawer-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><aside className="production-detail-drawer"><ProductionDetail order={selectedOrder} stages={selectedStages} plan={selectedPlan} qc={selectedQc} docs={selectedDocs} materials={selectedMaterials} busy={busy} onClose={onClose} onSetStage={onSetStage} onPlan={onPlan} onQc={onQc} onDocument={onDocument} onPrepareMaterials={onPrepareMaterials} onIssueAllMaterials={onIssueAllMaterials} onIssueMaterial={onIssueMaterial} onOutput={onOutput} /></aside></div>}
  </section>;
}
...
function ProductionDetail({ order, stages, plan, qc, docs, materials, busy, onClose, onSetStage, onPlan, onQc, onDocument, onPrepareMaterials, onIssueAllMaterials, onIssueMaterial, onOutput }) {
  ...
  return <section className="production-card detail-card"><CardTitle icon={FileText} title={`جزئیات ${order.code}`} action={<button onClick={onClose}>بستن ×</button>} />
    ...
    <section className="detail-block"><h3>مواد مصرفی تولید</h3>{materials?.length ? <div className="production-table-wrap"><table><thead><tr><th>فرمول/محصول</th><th>کد</th><th>کالا</th><th>درخواست</th><th>تأیید انبار</th><th>موجودی</th><th>وضعیت سند</th><th>عملیات</th></tr></thead><tbody>{materials.map(m => { const docStatus = m.warehouse_document_status || (m.status === 'issued' ? 'final' : ''); const issued = m.status === 'issued' || docStatus === 'final'; const requested = m.warehouse_document_id && !issued; const color = Number(m.bom_color_index || 1); return <tr key={m.id} className={`bom-color-row c${((color - 1) % 6) + 1}`}><td><span className={`bom-color-badge c${((color - 1) % 6) + 1}`}>{m.bom_product_name_fa || 'فرمول عمومی'}{m.bom_version_no ? ` · ${m.bom_version_no}` : ''}</span></td><td dir="ltr">{m.item_code}</td><td>{m.item_name_fa}</td><td>{formatNumber(m.quantity_requested)} {m.unit}</td><td>{m.quantity_issued ? `${formatNumber(m.quantity_issued)} ${m.unit}` : requested ? 'در انتظار تأیید انبار' : '—'}</td><td>{formatNumber(m.current_qty)}</td><td>{issued ? 'تأیید شده' : requested ? `${m.warehouse_document_number || 'سند موقت'} · در انتظار انبار` : 'آماده درخواست'}</td><td><button disabled={busy || issued || requested} onClick={() => onIssueMaterial(m)}>{issued ? 'خارج شد' : requested ? 'در انتظار انبار' : 'خروج از انبار'}</button></td></tr>; })}</tbody></table></div> : <p className="muted">مواد مصرفی هنوز آماده نشده است. با هر بار زدن «آماده‌سازی مواد»، آخرین فرمول‌های تولید مرتبط با این سفارش به این بخش منتقل می‌شود.</p>}</section><section className="detail-block"><h3>QC و اسناد</h3><div className="production-timeline compact">{qc.map((q) => <article key={q.id}><strong>{QC_LABELS[q.result] || q.result}</strong><small>{formatDateTime(q.checked_at)} · رد: {q.quantity_rejected || 0} · {q.rejection_reason || ''}</small></article>)}{docs.map((d) => <article key={d.id}><strong>{d.doc_number || '—'} · {d.title_fa}</strong><small>{d.document_type} · {formatDate(d.created_at)}</small></article>)}</div></section>
  </section>;
}
...
function PlanningSection({ orders, plans, busy, onPlan, onDeletePlan }) {
  ...
  const plannedOrders = useMemo(() => orders.filter((o) => {
    const p = planByOrderId[o.id];
    return Boolean(p || o.planned_start || o.planned_end);
...
function BomSection({ boms, bomItems, stock, orders, busy, onNew, onEdit, onSendFinance, onPublish }) {
  const materials = bomItems.filter(i => i.cost_type === 'material');
  return <section className="production-card"><div className="section-head"><CardTitle icon={FlaskConical} title="فرمول تولید کالا و هزینه‌ها" /><button onClick={onNew}>＋ فرمول جدید</button></div>{boms.length === 0 ? <Empty text="فرمول تولیدی ثبت نشده است." /> : <div className="bom-grid">{boms.map((b) => <article key={b.id} className="bom-card"><header><div><h3>{b.product_name_fa}</h3><p>{b.warehouse_item_code || 'کالای جدید/بدون کد'} · {b.version_no}</p></div><span>{b.status}</span></header><div className="bom-costs"><Info label="متریال" value={formatMoney(b.total_material_cost)} /><Info label="نیرو/خدمات" value={formatMoney(b.total_labor_cost)} /><Info label="سربار" value={formatMoney(b.total_overhead_cost)} /><Info label="جمع" value={formatMoney(b.total_estimated_cost)} /></div><small>{formatNumber(b.item_count)} ردیف هزینه</small><div className="row-actions"><button onClick={() => onEdit(b)}>ویرایش</button><button disabled={busy} onClick={() => onPublish(b)}>ثبت کالا/قیمت در انبار</button><button disabled={busy} onClick={() => onSendFinance(b)}>ارجاع هزینه به مالی</button></div></article>)}</div>}<section className="bom-materials-section"><CardTitle icon={Wrench} title="مواد و ابزارهای مصرفی تولید" />{materials.length?<div className="production-table-wrap"><table><thead><tr><th>فرمول</th><th>کالا/ابزار</th><th>کد انبار</th><th>تعداد</th><th>واحد</th><th>قیمت واحد</th><th>جمع</th></tr></thead><tbody>{materials.map(m=>{const bom=boms.find(b=>b.id===m.bom_id);const wh=stock.find(s=>s.item_id===m.warehouse_item_id);return <tr key={m.id}><td>{bom?.product_name_fa||'—'}</td><td>{m.item_name_fa}</td><td dir="ltr">{wh?.item_code||'—'}</td><td>{formatNumber(m.quantity)}</td><td>{m.unit}</td><td>{formatMoney(m.unit_cost)}</td><td>{formatMoney(m.total_cost)}</td></tr>})}</tbody></table></div>:<Empty text="متریال مصرفی ثبت نشده است."/>}</section></section>;
}
...
function DocumentsSection({ docs, orders, onNew, onVoid }) {
  ...
  const orderById = Object.fromEntries(orders.map((o) => [o.id, o]));
  const filtered=docs.filter(d=>(filters.type==='all'||d.document_type===filters.type)&&(filters.status==='all'||d.status===filters.status)&&(!filters.q||`${d.doc_number||''} ${d.title_fa||''} ${d.description_fa||''}`.includes(filters.q)));
  return <section className="production-card"><div className="section-head"><CardTitle icon={FileText} title="اسناد تولید" /><button onClick={onNew}>＋ سند تولید</button></div><div className="filters doc-filters"><input value={filters.q} onChange={e=>setFilters({...filters,q:e.target.value})} placeholder="جست‌وجوی سند..."/><select value={filters.type} onChange={e=>setFilters({...filters,type:e.target.value})}><option value="all">همه نوع‌ها</option><option value="instruction">دستور تولید</option><option value="material_issue">مصرف مواد</option><option value="qc_report">گزارش QC</option><option value="output">خروجی تولید</option><option value="cost">هزینه</option><option value="other">سایر</option></select><select value={filters.status} onChange={e=>setFilters({...filters,status:e.target.value})}><option value="all">همه وضعیت‌ها</option><option value="draft">پیش‌نویس</option><option value="registered">ثبت‌شده</option><option value="void">باطل</option></select></div>{filtered.length === 0 ? <Empty /> : <div className="production-table-wrap"><table><thead><tr><th>شماره</th><th>نوع</th><th>عنوان</th><th>تولید</th><th>وضعیت</th><th>تاریخ</th><th>عملیات</th></tr></thead><tbody>{filtered.map((d) => <tr key={d.id}><td dir="ltr">{d.doc_number}</td><td>{d.document_type}</td><td>{d.title_fa}</td><td>{orderById[d.production_order_id]?.code || '—'}</td><td>{d.status}</td><td>{formatDate(d.created_at)}</td><td><button disabled={d.status === 'void'} onClick={() => onVoid(d)}>ابطال</button></td></tr>)}</tbody></table></div>}</section>;
}
...
function BomModal({ bom, stock, orders = [], existingItems = [], busy, onClose, onSubmit }) {
  const [form, setForm] = useState({
    id: bom?.id || null,
    warehouse_item_id: bom?.warehouse_item_id || '',
    related_production_order_id: bom?.related_production_order_id || '',
    related_order_id: bom?.related_order_id || '',
    product_name_fa: bom?.product_name_fa || '',
    product_name_en: bom?.product_name_en || '',
    version_no: bom?.version_no || 'v1',
    status: bom?.status || 'draft',
    unit: bom?.unit || 'عدد',
    notes: bom?.notes || '',
  });
  const [items, setItems] = useState(existingItems.length ? existingItems.map((i) => ({
    ...
    notes: i.notes || '',
  })) : [{ cost_type: 'material', warehouse_item_id: '', item_name_fa: 'متریال تولید', quantity: 1, unit: 'عدد', unit_cost: 0, notes: '' }]);
  ...
  function selectStock(index, selected) {
    updateItem(index, {
      warehouse_item_id: selected?.item_id || '',
      item_name_fa: selected?.item_name_fa || items[index].item_name_fa,
      unit: selected?.unit || items[index].unit,
      unit_cost: selected?.unit_price_estimate ?? selected?.effective_sale_price ?? items[index].unit_cost,
    });
  }
  function selectOutputItem(selected) {
    setForm({
      ...form,
      warehouse_item_id: selected?.item_id || '',
      product_name_fa: selected?.item_name_fa || form.product_name_fa,
  ...
  function selectProductionOrder(id) {
    ...
    setForm({
      ...form,
      related_production_order_id: id,
      related_order_id: po?.source_order_id || form.related_order_id,
      product_name_fa: po?.product_name_fa || form.product_name_fa,
    });
  }
  return <Modal title={bom ? 'ویرایش فرمول تولید' : 'فرمول تولید جدید'} onClose={onClose}>
    <form className="bom-modal-form" onSubmit={(e) => { e.preventDefault(); onSubmit({ bom: form, items }); }}>
      <div className="form-grid bom-head-grid">
        <label><span>سفارش تولید مرتبط</span><select value={form.related_production_order_id} onChange={(e) => selectProductionOrder(e.target.value)}><option value="">بدون سفارش تولید</option>{orders.map((o) => <option key={o.id} value={o.id}>{o.code} · {o.product_name_fa}</option>)}</select></label>
        <label><span>کالای خروجی انبار</span><ProductPicker items={stock} value={form.warehouse_item_id} onSelect={selectOutputItem} placeholder="کد یا نام کالای خروجی را بنویس..." /></label>
        <label><span>نام محصول</span><input value={form.product_name_fa} onChange={(e) => setForm({ ...form, product_name_fa: e.target.value })} required /></label>
        <label><span>نسخه</span><input value={form.version_no} onChange={(e) => setForm({ ...form, version_no: e.target.value })} /></label>
        <label><span>وضعیت</span><select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}><option value="draft">پیش‌نویس</option><option value="active">فعال</option><option value="archived">آرشیو</option></select></label>
        <label className="full"><span>شرح</span><textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></label>
      </div>
      <div className="bom-lines-head"><b>ردیف‌های فرمول تولید</b><small>در ستون کد انبار، با نوشتن اولین حرف از کد یا نام کالا، لیست کالاها فیلتر می‌شود.</small></div>
      <div className="line-editor bom-line-editor"><table><thead><tr><th>نوع هزینه</th><th>کد/نام کالا</th><th>شرح</th><th>تعداد</th><th>واحد</th><th>قیمت واحد ریال</th><th>جمع</th><th></th></tr></thead><tbody>{items.map((item, index) => <tr key={index}><td><select value={item.cost_type} onChange={(e) => updateItem(index, { cost_type: e.target.value })}>{Object.entries(COST_TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></td><td><ProductPicker items={stock} value={item.warehouse_item_id || ''} onSelect={(selected) => selectStock(index, selected)} placeholder="کد یا نام متریال..." /></td><td><input value={item.item_name_fa} onChange={(e) => updateItem(index, { item_name_fa: e.target.value })} /></td><td><input type="number" value={item.quantity} onChange={(e) => updateItem(index, { quantity: e.target.value })} /></td><td><input value={item.unit} onChange={(e) => updateItem(index, { unit: e.target.value })} /></td><td><input type="number" value={item.unit_cost} onChange={(e) => updateItem(index, { unit_cost: e.target.value })} /></td><td>{formatMoney(Number(item.quantity || 0) * Number(item.unit_cost || 0))}</td><td><button type="button" onClick={() => setItems((rows) => rows.filter((_, i) => i !== index))}>×</button></td></tr>)}</tbody></table></div>
      <button type="button" className="mini-btn" onClick={() => setItems((rows) => [...rows, { cost_type: 'material', warehouse_item_id: '', item_name_fa: 'ردیف جدید', quantity: 1, unit: 'عدد', unit_cost: 0 }])}>＋ افزودن ردیف</button>
      <div className="form-summary"><b>جمع کل برآوردی: {formatMoney(total)}</b></div>
      <div className="modal-actions sticky-modal-actions"><button type="button" onClick={onClose}>انصراف</button><button disabled={busy} type="submit">ذخیره فرمول</button></div>
    </form>
  </Modal>;
}
...
function OutputModal({ order, stock, existingOutputs = [], busy, onClose, onSubmit }) {
  const hasOutput = existingOutputs.length > 0 || Number(order?.output_registered_qty || order?.quantity_produced || 0) > 0;
  ...
  const initialRows = existingOutputs.length
    ? existingOutputs.map((out) => ({
        warehouseItemId: out.warehouse_item_id || '',
        productNameFa: out.warehouse_items?.item_name_fa || '',
        quantity: out.quantity || 1,
        unit: out.warehouse_items?.unit || order?.unit || 'عدد',
        unitPriceEstimate: 0,
      }))
    : [{ warehouseItemId: order?.output_warehouse_item_id || '', productNameFa: order?.product_name_fa || '', quantity: order?.quantity_planned || 1, unit: order?.unit || 'عدد', unitPriceEstimate: order?.estimated_total_cost || 0 }];
  const [rows,setRows]=useState(initialRows);
  const title = hasOutput ? 'اصلاح خروجی‌های ثبت‌شده در انبار' : 'ثبت خروجی تولید در انبار';
  function update(index, patch){setRows((list)=>list.map((row,i)=>i===index?{...row,...patch}:row));}
  function selectItem(index, selected){update(index,{warehouseItemId:selected?.item_id||'',productNameFa:selected?.item_name_fa||rows[index]?.productNameFa||'',unit:selected?.unit||rows[index]?.unit||'عدد',unitPriceEstimate:selected?.unit_price_estimate||rows[index]?.unitPriceEstimate||0});}
  function addRow(){setRows((list)=>[...list,{warehouseItemId:'',productNameFa:'',quantity:1,unit:order?.unit||'عدد',unitPriceEstimate:0}]);}
  const validRows = rows.filter((row)=>Number(row.quantity||0)>0 && (row.warehouseItemId || row.productNameFa?.trim()));
  return <Modal title={title} onClose={onClose}><form onSubmit={(e)=>{e.preventDefault(); if (hasOutput && !confirmed) return; onSubmit({productionOrderId: order?.id, outputs: validRows})}}>
    {hasOutput && !confirmed && <div className="confirm-box production-confirm"><b>این تولید قبلاً خروجی انبار دارد.</b><p>برای این سفارش {formatNumber(existingOutputs.length || 1)} ردیف خروجی ثبت شده است. آیا می‌خواهید مقدار/محصولات خروجی را اصلاح کنید؟</p><div className="confirm-actions"><button type="button" onClick={onClose}>خیر</button><button type="button" onClick={()=>setConfirmed(true)}>بله، اصلاح خروجی‌ها</button></div></div>}
    {(!hasOutput || confirmed) && <><div className="output-lines-editor"><table><thead><tr><th>کالای انبار</th><th>نام محصول جدید/خروجی</th><th>تعداد</th><th>واحد</th><th>قیمت مرجع ریال</th><th></th></tr></thead><tbody>{rows.map((row,index)=><tr key={index}><td><ProductPicker items={finished} value={row.warehouseItemId} onSelect={(selected)=>selectItem(index,selected)} placeholder="کد یا نام محصول تولیدی..." /></td><td><input value={row.productNameFa} onChange={e=>update(index,{productNameFa:e.target.value})} placeholder="مثلاً X1 یا X2"/></td><td><input type="number" value={row.quantity} onChange={e=>update(index,{quantity:e.target.value})} required/></td><td><input value={row.unit} onChange={e=>update(index,{unit:e.target.value})}/></td><td><input type="number" value={row.unitPriceEstimate} onChange={e=>update(index,{unitPriceEstimate:e.target.value})}/></td><td><button type="button" disabled={rows.length===1} onClick={()=>setRows((list)=>list.filter((_,i)=>i!==index))}>×</button></td></tr>)}</tbody></table><button type="button" className="mini-btn" onClick={addRow}>＋ افزودن خروجی محصول</button></div><div className="note">برای یک سفارش می‌توانید چند خروجی محصول مثل X1 و X2 ثبت کنید. اگر کالای انبار انتخاب نشود، سیستم با نام محصول واردشده یک کالای تولیدی جدید با کد چهاررقمی می‌سازد.</div><div className="modal-actions"><button type="button" onClick={onClose}>انصراف</button><button disabled={busy || validRows.length===0} type="submit">{hasOutput ? 'ثبت اصلاح خروجی‌ها' : 'ثبت خروجی‌ها در انبار'}</button></div></>}
  </form></Modal>}
