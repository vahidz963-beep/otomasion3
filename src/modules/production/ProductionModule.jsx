...
function FlowSection({ orders, stages, selectedOrder, selectedStages, selectedPlan, selectedQc, selectedDocs, query, setQuery, busy, onSelect, onClose, onSetStage, onPlan, onQc, onDocument, onOutput, onExport, onPrint }) {
  return <section className="production-card production-flow-workspace">
    ...
    {selectedOrder && <div className="production-drawer-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><aside className="production-detail-drawer"><ProductionDetail order={selectedOrder} stages={selectedStages} plan={selectedPlan} qc={selectedQc} docs={selectedDocs} busy={busy} onClose={onClose} onSetStage={onSetStage} onPlan={onPlan} onQc={onQc} onDocument={onDocument} onOutput={onOutput} /></aside></div>}
  </section>;
}

function ProductionDetail({ order, stages, plan, qc, docs, busy, onClose, onSetStage, onPlan, onQc, onDocument, onOutput }) {
  return <section className="production-card detail-card"><CardTitle icon={FileText} title={`جزئیات ${order.code}`} action={<button onClick={onClose}>بستن ×</button>} />
    ...
    <div className="detail-actions"><button onClick={() => onPlan(order)}>برنامه‌ریزی</button><button onClick={() => onQc(order)}>ثبت QC</button><button onClick={() => onDocument(order)}>سند تولید</button><button onClick={() => onOutput(order)}>ثبت خروجی به انبار</button></div>
