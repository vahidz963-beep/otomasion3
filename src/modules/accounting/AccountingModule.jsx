...
export default function AccountingModule({ lang = 'fa' }) {
  ...
  return (
    <div className="accounting-page" dir={dir} lang={lang}>
      ...
      {!data.loading && tab === 'cashflow' && <CashFlowSection accounts={data.treasuryAccounts} bankAccounts={data.bankAccounts} ledger={data.paymentLedger} investments={data.investments} lang={lang} onNewPayment={() => setModal('payment')} onNewIncomeExpense={(direction) => setModal({ type: 'incomeExpense', direction })} onEditPayment={openPaymentEdit} onNewAccount={() => setAccountModal({})} onEditAccount={(account) => setAccountModal({ account })} onDeleteAccount={(account) => setAccountDeleteModal({ account })} isAdmin={isAdmin} onNewInvestment={() => setInvestmentModal({})} onEditInvestment={(item) => setInvestmentModal({ item })} onArchiveInvestment={(item) => runAction(() => archiveFinanceInvestment(item.id), 'سرمایه‌گذاری آرشیو شد.')} />}
...
function CashFlowSection({ accounts, bankAccounts = [], ledger, investments, lang, onNewPayment, onNewIncomeExpense, onEditPayment, onNewAccount, onEditAccount, onDeleteAccount, isAdmin, onNewInvestment, onEditInvestment, onArchiveInvestment }) {
  ...
  const ledgerWithBalances = useMemo(() => {
    ...
    const chronological = [...(ledger || [])].sort((a, b) => `${a.payment_date || ''} ${a.created_at || ''}`.localeCompare(`${b.payment_date || ''} ${b.created_at || ''}`));
