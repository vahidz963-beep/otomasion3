...
import { formatJalaliDate, formatToman, gregorianToJalali, loadCurrencyDisplayPreference, setCurrencyDisplayPreference, toEnglishDigits, toPersianDigits } from '../../lib/formatters';
...
import {
  ...
  createFinanceFiscalYear,
  fetchFinanceBalanceSheet,
  getFinancePaymentForEdit,
  ...
  createFinanceIncomeExpenseCategory,
  updateFinanceIncomeExpenseCategory,
  archiveFinanceIncomeExpenseCategory,
  createFinanceLoan,
  updateFinanceLoan,
  archiveFinanceLoan,
...
const COPY = {
  fa: {
    ...
    tabs: {
      ...
      incomeExpense: 'هزینه‌ها و درآمدها',
      checks: 'چک‌ها',
  ...
  en: {
    ...
    refresh: 'Refresh',
    tabs: { overview: 'Overview', documents: 'Invoices', parties: 'Parties', cashflow: 'Cash & Bank', incomeExpense: 'Income & Expense', checks: 'Checks', itemKardex: 'Item Kardex', profitability: 'Order Profit', payroll: 'Payroll', loans: 'Loans', fiscal: 'Fiscal', referrals: 'Referrals', settings: 'Settings' },
...
export default function AccountingModule({ lang = 'fa' }) {
  ...
  const [currencyDisplay, setCurrencyDisplay] = useState(() => loadCurrencyDisplayPreference(profile?.id || 'default'));
  useEffect(() => { setCurrencyDisplay(loadCurrencyDisplayPreference(profile?.id || 'default')); }, [profile?.id]);
  function changeCurrencyDisplay(value) { setCurrencyDisplayPreference(value, profile?.id || 'default'); setCurrencyDisplay(value); }
  const t = COPY[lang];
  ...
  const [incomeExpenseCategoryModal, setIncomeExpenseCategoryModal] = useState(null);
  const [incomeExpensePaymentDeleteModal, setIncomeExpensePaymentDeleteModal] = useState(null);
  ...
  async function runAction(fn, successMessage) {
    ...
    try {
      ...
      setIncomeExpenseCategoryModal(null);
      setIncomeExpensePaymentDeleteModal(null);
      setLoanModal(null);
  ...
  const submitPayment = (payload) => runAction(() => payload.paymentId ? updateFinancePayment(payload.paymentId, { ...payload, reason: 'ویرایش از صندوق و گردش حساب / هزینه‌ها و درآمدها' }) : createFinancePayment(payload), payload.paymentId ? 'دریافت/پرداخت ویرایش و سند حسابداری آن به‌روز شد.' : 'دریافت/پرداخت ثبت و سند حسابداری آن ایجاد شد.');
  async function openPaymentEdit(paymentRow) {
    if (!paymentRow?.id) return;
    setBusy(true);
    setNotice('');
    try {
      const payment = await getFinancePaymentForEdit(paymentRow.id);
      if (payment.source_record_id) {
        setNotice('این سند از بخش دیگری مثل چک، حقوق یا اقساط ساخته شده است. برای جلوگیری از خراب شدن سوابق، ویرایش مبلغ/ماهیت آن باید از همان بخش اصلی انجام شود.');
        return;
      }
      const modalType = payment.category_id || payment.source_module === 'accounting' ? 'incomeExpense' : 'payment';
      setModal({ type: modalType, direction: payment.direction, payment });
    } catch (e) {
      setNotice(getFriendlyErrorMessage(e, 'خطا در آماده‌سازی سند برای ویرایش'));
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="accounting-page" dir={dir} lang={lang}>
      <header className="accounting-hero">
        ...
        <div className="hero-actions">
          ...
          <button type="button" onClick={() => setModal({ type: 'incomeExpense', direction: 'payment' })}><WalletCards size={16} /> هزینه/درآمد</button>
      ...
      {modal && (() => {
        ...
        return (
          <FinanceModal title={modalTitle(modalType)} onClose={() => setModal(null)}>
            ...
            {modalType === 'payment' && <FinancePaymentForm parties={data.parties} documents={activeDocuments} accounts={data.bankAccounts} cashboxes={data.cashboxes || []} loans={data.loans || []} categories={data.incomeExpenseCategories || []} initialDocumentId={modalDocumentId} initialPayment={typeof modal === 'object' ? modal.payment : null} busy={busy} onCancel={() => setModal(null)} onSubmit={submitPayment} />}
            {modalType === 'incomeExpense' && <FinancePaymentForm parties={data.parties} documents={[]} accounts={data.bankAccounts} cashboxes={data.cashboxes || []} loans={data.loans || []} categories={data.incomeExpenseCategories || []} initialDirection={(typeof modal === 'object' && modal.direction) || 'payment'} initialPayment={typeof modal === 'object' ? modal.payment : null} categoryMode busy={busy} onCancel={() => setModal(null)} onSubmit={submitPayment} />}
      ...
      {incomeExpenseCategoryModal && <IncomeExpenseCategoryModal initial={incomeExpenseCategoryModal.category} type={incomeExpenseCategoryModal.type} categories={data.incomeExpenseCategories || []} busy={busy} onClose={() => setIncomeExpenseCategoryModal(null)} onSubmit={(payload) => runAction(() => incomeExpenseCategoryModal.category ? updateFinanceIncomeExpenseCategory(incomeExpenseCategoryModal.category.id, payload) : createFinanceIncomeExpenseCategory(payload), incomeExpenseCategoryModal.category ? 'دسته ویرایش شد.' : 'دسته هزینه/درآمد ثبت شد.')} onArchive={(category) => runAction(() => archiveFinanceIncomeExpenseCategory(category.id), 'دسته غیرفعال شد.')} />}
      {incomeExpensePaymentDeleteModal && <IncomeExpensePaymentDeleteModal payment={incomeExpensePaymentDeleteModal.payment} busy={busy} onClose={() => setIncomeExpensePaymentDeleteModal(null)} onConfirm={(reason) => runAction(() => voidFinancePayment(incomeExpensePaymentDeleteModal.payment.id, reason), 'سند هزینه/درآمد باطل و از لیست فعال حذف شد.')} />}
      ...
      {!data.loading && tab === 'cashflow' && <CashFlowSection accounts={data.treasuryAccounts} bankAccounts={data.bankAccounts} ledger={data.paymentLedger} investments={data.investments} lang={lang} onNewPayment={() => setModal('payment')} onDeletePayment={(payment) => { if (window.confirm(`سند ${payment.payment_number || ''} باطل و از لیست فعال حذف شود؟ این عملیات سابقه حسابداری را حفظ می‌کند و مبلغ دیگر در مانده حساب محاسبه نمی‌شود.`)) runAction(() => voidFinancePayment(payment.id, 'ابطال از صندوق و گردش حساب'), 'سند باطل و از لیست فعال حذف شد.'); }} onNewIncomeExpense={(direction) => setModal({ type: 'incomeExpense', direction })} onEditPayment={openPaymentEdit} onNewAccount={() => setAccountModal({})} onEditAccount={(account) => setAccountModal({ account })} onDeleteAccount={(account) => setAccountDeleteModal({ account })} isAdmin={isAdmin} onNewInvestment={() => setInvestmentModal({})} onEditInvestment={(item) => setInvestmentModal({ item })} onArchiveInvestment={(item) => runAction(() => archiveFinanceInvestment(item.id), 'سرمایه‌گذاری آرشیو شد.')} />}
      {!data.loading && tab === 'incomeExpense' && <IncomeExpenseSection categories={data.incomeExpenseCategories || []} ledger={data.incomeExpenseLedger || []} lang={lang} onNewCategory={(type) => setIncomeExpenseCategoryModal({ type })} onEditCategory={(category) => setIncomeExpenseCategoryModal({ category, type: category.category_type })} onNewPayment={(direction) => setModal({ type: 'incomeExpense', direction })} onEditPayment={(payment) => setModal({ type: 'incomeExpense', direction: payment.direction, payment })} onVoidPayment={(payment) => setIncomeExpensePaymentDeleteModal({ payment })} />}
      ...
      {!data.loading && tab === 'fiscal' && <FiscalSection fiscalYears={data.fiscalYears} fiscalPeriods={data.fiscalPeriods} lang={lang} busy={busy} onCreateYear={(payload) => runAction(() => createFinanceFiscalYear(payload), 'سال مالی جدید ثبت شد و ۱۲ ماه آن به‌صورت خودکار ساخته شد.')} onClosePeriod={(id) => runAction(() => closeFiscalPeriod(id), 'ماه مالی بسته شد.')} onReopenPeriod={(id) => runAction(() => reopenFiscalPeriod(id), 'ماه مالی بازگشایی شد.')} onCloseYear={(id) => runAction(() => closeFiscalYear(id), 'سال مالی و همه ماه‌های آن بسته شد.')} onReopenYear={(id) => runAction(() => reopenFiscalYear(id), 'سال مالی و همه ماه‌های آن بازگشایی شد.')} />}
      ...
      {!data.loading && tab === 'settings' && <SettingsSection numbering={data.numbering} ioDocuments={data.ioDocuments} lang={lang} busy={busy} currencyDisplay={currencyDisplay} onCurrencyDisplayChange={changeCurrencyDisplay} onUpdateNumbering={(ruleKey, patch) => runAction(() => updateNumberingRule(ruleKey, patch), 'تنظیمات شماره‌گذاری ذخیره شد.')} onAddIo={(type) => runAction(() => createIoDocument({ io_type: type, title_fa: type === 'incoming' ? 'سند ورودی نمونه' : 'سند خروجی نمونه', source_module: 'accounting', status: 'registered' }), 'سند ورودی/خروجی ثبت شد.')} />}
    </div>
  );
}
...
function BalanceTable({ parties, lang }) {
  ...
  const th = (key, label) => <th><button className="sort-th" onClick={() => setSort(nextSort(sort, key))}>{label}<span>{sort.key === key ? (sort.dir === 'asc' ? '▲' : '▼') : '↕'}</span></button></th>;
  return <div className="table-scroll limited-list"><table className="finance-table"><thead><tr>{th('display_name','شخص')}{th('customer_code','کد مشتری')}{th('party_type','نوع')}{th('balance','وضعیت')}{th('total_debit','بدهکار')}{th('total_credit','بستانکار')}{th('balance','مانده')}</tr></thead><tbody>{rows.map((p) => <tr key={p.party_id}><td>{p.display_name}</td><td dir="ltr">{p.customer_code || '—'}</td><td>{partyTypeLabel(p.party_type, lang)}</td><td>{Number(p.balance) > 0 ? 'بدهکار' : 'بستانکار'}</td><td>{Number(p.balance) > 0 ? formatMoney(p.balance, lang) : '—'}</td><td>{Number(p.balance) < 0 ? formatMoney(Math.abs(p.balance), lang) : '—'}</td><td className={Number(p.balance) >= 0 ? 'positive' : 'negative'}>{formatMoney(p.balance, lang)}</td></tr>)}</tbody></table></div>;
}

function CashFlowSection({ accounts, bankAccounts = [], ledger, investments, lang, onNewPayment, onNewIncomeExpense, onEditPayment, onDeletePayment, onNewAccount, onEditAccount, onDeleteAccount, isAdmin, onNewInvestment, onEditInvestment, onArchiveInvestment }) {
  ...
  const ledgerWithBalances = useMemo(() => {
    const balances = new Map((accounts || []).map((a) => [String(a.account_id), Number(a.opening_balance || 0)]));
    ...
    chronological.forEach((row) => {
      ...
      const isTransfer = row.method === 'account_transfer';
      const sourceDelta = isTransfer || row.direction === 'payment' ? -amount : amount;
      const sourceBalance = (balances.get(source) || 0) + sourceDelta;
    ...
    return filtered.map((row) => ({ ...row, running_balance: calculated.get(row.id) ?? null }));
  }, [accounts, ledger, filtered]);
  const th = (key, label) => <th><button className="sort-th" onClick={() => setSort(nextSort(sort, key))}>{label}<span>{sort.key === key ? (sort.dir === 'asc' ? '▲' : '▼') : '↕'}</span></button></th>;
  return <div className="accounting-grid cashflow-layout finance-cashflow-full">
    <section className="finance-card cashflow-main-card full-width"><div className="finance-card-header between"><CardHeader icon={Banknote} title="صندوق و گردش حساب" bare /><div className="actions-cell"><button className="mini-btn" onClick={onNewAccount}>＋ افزودن حساب/کارت</button><button className="mini-btn" onClick={onNewPayment}>＋ دریافت/پرداخت</button><button className="mini-btn primary-soft" onClick={() => onNewIncomeExpense?.('payment')}>＋ هزینه/درآمد</button></div></div><div className="cash-account-grid bank-card-grid">{enrichedAccounts.map((a) => <BankFlipCard key={`${a.account_kind}-${a.account_id}`} account={a} lang={lang} onEdit={onEditAccount} onDelete={onDeleteAccount} isAdmin={isAdmin} />)}</div><div className="toolbar-line"><select value={filters.accountId} onChange={(e)=>setFilters({...filters,accountId:e.target.value})}><option value="all">همه حساب‌ها</option>{enrichedAccounts.map(a=><option key={a.account_id} value={a.account_id}>{a.account_name}</option>)}</select><select value={filters.direction} onChange={(e)=>setFilters({...filters,direction:e.target.value})}><option value="all">همه گردش‌ها</option><option value="receipt">واریزی</option><option value="payment">برداشت</option></select><input value={filters.q} onChange={(e)=>setFilters({...filters,q:e.target.value})} placeholder="جست‌وجوی شماره، شخص، شرح..." /></div><div className="table-scroll limited-list tall"><table className="finance-table cashflow-ledger-table"><thead><tr>{th('payment_date','تاریخ')}{th('payment_number','شماره')}{th('account_name','حساب')}{th('party_name','شخص')}{th('direction','نوع')}{th('method','روش')}{th('amount','مبلغ')}<th>مانده حساب</th><th>شرح</th><th>ویرایش</th></tr></thead><tbody>{ledgerWithBalances.map(r=><tr key={r.id}><td>{formatDate(r.payment_date, lang)}</td><td dir="ltr">{r.payment_number}</td><td>{r.account_name || '—'}</td><td>{r.party_name || '—'}</td><td className={r.direction==='receipt'?'receipt-text':'payment-text'}>{r.direction === 'receipt' ? 'واریز' : 'برداشت'}</td><td>{r.method}</td><td className={r.direction==='receipt'?'receipt-text':'payment-text'}>{formatMoney(r.amount, lang)}</td><td className="running-balance-cell">{r.running_balance === null ? '—' : formatMoney(r.running_balance, lang)}</td><td>{r.description || '—'}</td><td className="cashflow-edit-cell"><button type="button" className="row-round-edit-btn" title="ویرایش سند دریافت/پرداخت" aria-label="ویرایش سند دریافت/پرداخت" onClick={() => onEditPayment?.(r)}><Edit3 size={14} /></button><button type="button" className="row-round-edit-btn danger" title="باطل/حذف از لیست فعال" aria-label="باطل/حذف از لیست فعال" onClick={() => onDeletePayment?.(r)}><Trash2 size={14} /></button></td></tr>)}</tbody></table></div></section>
...
function getJalaliMonthKey(value) {
  if (!value) return '';
  const clean = value instanceof Date ? value : String(value).slice(0, 10);
  const date = clean instanceof Date ? clean : new Date(`${clean}T00:00:00`);
  if (Number.isNaN(date.getTime())) return '';
  const j = gregorianToJalali(date.getFullYear(), date.getMonth() + 1, date.getDate());
  return `${j.jy}/${String(j.jm).padStart(2, '0')}`;
}

function IncomeExpenseSection({ categories = [], ledger = [], lang, onNewCategory, onEditCategory, onNewPayment, onEditPayment, onVoidPayment }) {
  ...
  const monthOptions = useMemo(() => {
    const keys = new Set((ledger || []).map((row) => getJalaliMonthKey(row.payment_date)).filter(Boolean));
    const current = getJalaliMonthKey(new Date().toISOString().slice(0, 10));
    if (current) keys.add(current);
    return [...keys].sort((a, b) => b.localeCompare(a, 'fa'));
  }, [ledger]);
  const monthRows = useMemo(() => (ledger || []).filter((row) => monthFilter === 'all' || getJalaliMonthKey(row.payment_date) === monthFilter), [ledger, monthFilter]);
  const monthlyTotals = useMemo(() => monthRows.reduce((acc, row) => {
    ...
    return acc;
  }, { income: 0, expense: 0 }), [monthRows]);
  const filteredLedger = useMemo(() => sortRows(monthRows.filter((row) => {
    const flow = row.flow_type || row.category_type || (row.direction === 'receipt' ? 'income' : 'expense');
    const text = `${row.payment_number || ''} ${row.party_name || ''} ${row.subject || ''} ${row.category_name_fa || ''} ${row.parent_category_name_fa || ''} ${row.description || ''} ${row.category_note || ''} ${row.order_code || ''}`.toLowerCase();
...
function CategoryCard({ title, type, rows, onNew, onEdit }) {
  const parents = rows.filter((r) => !r.parent_id);
  const childrenByParent = rows.reduce((map, row) => { if (row.parent_id) { if (!map[row.parent_id]) map[row.parent_id] = []; map[row.parent_id].push(row); } return map; }, {});
  return <section className={`finance-card category-card ${type}`}><div className="finance-card-header between"><CardHeader icon={type === 'expense' ? ArrowUpRight : ArrowDownLeft} title={title} bare /><button className="mini-btn" onClick={onNew}>＋ دسته جدید</button></div>{rows.length === 0 ? <Empty t={{ noData: 'دسته‌ای ثبت نشده است.' }} /> : <div className="category-list">{parents.map((cat) => <article key={cat.id} className="category-item"><div><b>{cat.name_fa}</b><small>{cat.code || '—'} {cat.notes ? `· ${cat.notes}` : ''}</small></div><button onClick={() => onEdit(cat)}>ویرایش</button>{(childrenByParent[cat.id] || []).map((child) => <div key={child.id} className="category-child"><span>{child.name_fa}</span><button onClick={() => onEdit(child)}>ویرایش</button></div>)}</article>)}</div>}</section>;
}

function IncomeExpenseCategoryModal({ initial, type = 'expense', categories = [], busy, onClose, onSubmit, onArchive }) {
  const [form, setForm] = useState({ category_type: initial?.category_type || type || 'expense', parent_id: initial?.parent_id || '', code: initial?.code || '', name_fa: initial?.name_fa || '', name_en: initial?.name_en || '', notes: initial?.notes || '', is_active: initial?.is_active !== false });
  const parentOptions = categories.filter((c) => c.category_type === form.category_type && c.id !== initial?.id && !c.parent_id);
  return <FinanceModal title={initial ? 'ویرایش دسته هزینه/درآمد' : `دسته جدید ${form.category_type === 'income' ? 'درآمد' : 'هزینه'}`} onClose={onClose}><div className="finance-form-grid"><label className="finance-field"><span>نوع دسته</span><select value={form.category_type} onChange={(e)=>setForm({...form, category_type:e.target.value, parent_id:''})}><option value="expense">هزینه</option><option value="income">درآمد</option></select></label><label className="finance-field"><span>زیرمجموعه دسته</span><select value={form.parent_id} onChange={(e)=>setForm({...form,parent_id:e.target.value})}><option value="">دسته اصلی</option>{parentOptions.map((c)=><option key={c.id} value={c.id}>{c.name_fa}</option>)}</select></label><label className="finance-field"><span>کد اختیاری</span><input dir="ltr" value={form.code} onChange={(e)=>setForm({...form,code:e.target.value})} placeholder="EXP-REPAIR" /></label><label className="finance-field"><span>عنوان</span><input value={form.name_fa} onChange={(e)=>setForm({...form,name_fa:e.target.value})} required placeholder="مثلاً هزینه تعمیر دستگاه" /></label><label className="finance-field full"><span>توضیحات</span><textarea value={form.notes} onChange={(e)=>setForm({...form,notes:e.target.value})} /></label></div><div className="finance-note warning">این دسته‌ها در فرم دریافت/پرداخت قابل انتخاب هستند و اسناد هزینه/درآمد را در بخش «هزینه‌ها و درآمدها» دسته‌بندی می‌کنند.</div><div className="finance-form-actions"><button onClick={onClose}>انصراف</button>{initial && <button className="danger-btn" disabled={busy} onClick={()=>onArchive?.(initial)}>غیرفعال</button>}<button disabled={busy || !form.name_fa.trim()} onClick={()=>onSubmit(form)}>{initial ? 'ذخیره ویرایش' : 'ثبت دسته'}</button></div></FinanceModal>;
}


function IncomeExpensePaymentDeleteModal({ payment, busy, onClose, onConfirm }) {
  const [reason, setReason] = useState('حذف/ابطال سند هزینه یا درآمد با تأیید حسابداری');
  const isIncome = payment?.direction === 'receipt' || payment?.flow_type === 'income';
  return <FinanceModal title="حذف / ابطال سند هزینه و درآمد" onClose={onClose}>
    <div className="confirm-finance income-expense-delete-confirm">
      <p>آیا از حذف امن این سند مطمئن هستید؟</p>
      <div className="finance-note warning">حذف به معنی پاک‌کردن کامل نیست؛ برای حفظ سوابق مالی، سند «باطل» می‌شود و سند حسابداری قبلی هم از حالت فعال خارج می‌شود.</div>
      <div className="delete-summary-box">
        <span>شماره: <b dir="ltr">{payment?.payment_number || '—'}</b></span>
        <span>نوع: <b>{isIncome ? 'درآمد / دریافت' : 'هزینه / پرداخت'}</b></span>
        <span>تاریخ: <b>{formatDate(payment?.payment_date, 'fa')}</b></span>
        <span>مبلغ: <b>{formatMoney(payment?.amount || 0, 'fa')}</b></span>
      </div>
      <textarea value={reason} onChange={(e)=>setReason(e.target.value)} placeholder="دلیل حذف/ابطال را بنویسید..." />
      <div className="finance-form-actions"><button type="button" onClick={onClose}>انصراف</button><button className="danger-btn" disabled={busy || !reason.trim()} onClick={()=>onConfirm(reason)}>تأیید حذف امن</button></div>
    </div>
  </FinanceModal>;
}
...
function LoanPaymentModal({ installment, payments = [], busy, onClose, onSubmit }) {
  ...
  const remaining = Math.max(Number(installment?.amount_due || 0) - Number(installment?.paid_amount || 0), 0);
  const [paymentMode, setPaymentMode] = useState('full');
  const [form, setForm] = useState({ paymentId: '', paidAmount: remaining, paidAt: new Date().toISOString().slice(0,10), notes: '' });
  return <FinanceModal title={`ثبت پرداخت قسط ${installment?.installment_no || ''}`} onClose={onClose}><div className="finance-form-grid"><label className="finance-field"><span>نوع پرداخت</span><select value={paymentMode} onChange={(e)=>{const mode=e.target.value;setPaymentMode(mode);if(mode==='full')setForm({...form,paidAmount:remaining});}}><option value="full">پرداخت کامل قسط</option><option value="custom">پرداخت مبلغ دلخواه</option></select></label><label className="finance-field"><span>قسط شروع پرداخت</span><input readOnly value={`${installment?.loan_title || 'وام'} · قسط ${installment?.installment_no || '—'} · مانده ${formatMoney(remaining, 'fa')}`} /></label><label className="finance-field"><span>مبلغ پرداختی ریال</span><MoneyInput value={form.paidAmount} onChange={(value)=>setForm({...form,paidAmount:value})}/></label><label className="finance-field"><span>تاریخ پرداخت</span><JalaliDateInput value={form.paidAt} onChange={(v)=>setForm({...form,paidAt:v})}/></label><label className="finance-field full"><span>سند پرداخت بانک/صندوق</span><select value={form.paymentId} onChange={(e)=>setForm({...form,paymentId:e.target.value})}><option value="">بدون انتخاب سند پرداخت</option>{payablePayments.map((p)=><option key={p.id} value={p.id}>{p.payment_number} · {p.account_name || '—'} · {formatMoney(p.amount, 'fa')} · {formatDate(p.payment_date, 'fa')}</option>)}</select></label><label className="finance-field full"><span>یادداشت</span><textarea value={form.notes} onChange={(e)=>setForm({...form,notes:e.target.value})}/></label></div><div className="finance-form-actions"><button type="button" onClick={onClose}>انصراف</button><button disabled={busy} onClick={()=>onSubmit({ loanId: installment.loan_id, installmentId: installment.id, paymentId: form.paymentId || null, paidAmount: form.paidAmount, paidAt: form.paidAt, notes: form.notes })}>تأیید پرداخت قسط</button></div></FinanceModal>;
}
...
function FiscalStatus({ closed }) {
  return <span className={closed ? 'fiscal-status closed' : 'fiscal-status open'}>{closed ? 'بسته' : 'باز'}</span>;
}

function FiscalSection({ fiscalYears = [], fiscalPeriods = [], lang, busy, onCreateYear, onClosePeriod, onReopenPeriod, onCloseYear, onReopenYear }) {
  const [selectedYearId, setSelectedYearId] = useState(fiscalYears[0]?.id || '');
  const [addingYear, setAddingYear] = useState(false);
  const years = useMemo(() => sortRows(fiscalYears || [], { key: 'start_date', dir: 'desc' }), [fiscalYears]);
  const selectedYear = years.find((y) => y.id === selectedYearId) || years[0] || null;
  const periods = useMemo(() => sortRows((fiscalPeriods || []).filter((p) => !selectedYear?.id || p.fiscal_year_id === selectedYear.id), { key: 'period_no', dir: 'asc' }), [fiscalPeriods, selectedYear?.id]);
  const closedMonths = periods.filter((p) => p.is_closed).length;
  return <div className="accounting-grid fiscal-layout-grid">
    {addingYear && <FiscalYearModal fiscalYears={years} busy={busy} onClose={() => setAddingYear(false)} onSubmit={async (payload) => { await onCreateYear?.(payload); setAddingYear(false); }} />}
    <section className="finance-card fiscal-years-card">
      <div className="finance-card-header between"><CardHeader icon={CalendarClock} title="سال‌های مالی" bare /><button className="mini-btn primary-soft" disabled={busy} onClick={() => setAddingYear(true)}>＋ افزودن سال مالی</button></div>
      <p className="finance-note">با افزودن سال مالی، ۱۲ ماه همان سال به‌صورت خودکار ساخته می‌شود. بستن سال، همه ماه‌های آن را هم می‌بندد؛ بازگشایی سال هم همه ماه‌های آن را باز می‌کند.</p>
      {years.length === 0 ? <Empty t={{ noData: 'سال مالی تعریف نشده است.' }} /> : <div className="table-scroll limited-list fiscal-years-list"><table className="finance-table compact fiscal-years-table"><thead><tr><th>عنوان</th><th>شروع</th><th>پایان</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>{years.map((year) => <tr key={year.id} className={selectedYear?.id === year.id ? 'selected-row clickable-row' : 'clickable-row'} onClick={() => setSelectedYearId(year.id)}><td><b>{year.title}</b></td><td>{formatDate(year.start_date, lang)}</td><td>{formatDate(year.end_date, lang)}</td><td><FiscalStatus closed={year.is_closed} /></td><td className="actions-cell" onClick={(e)=>e.stopPropagation()}><button className="mini-btn" disabled={busy || year.is_closed} onClick={() => onCloseYear(year.id)}>بستن سال</button><button className="mini-btn" disabled={busy || !year.is_closed} onClick={() => onReopenYear(year.id)}>بازگشایی سال</button></td></tr>)}</tbody></table></div>}
    </section>
    <section className="finance-card fiscal-periods-card">
      <div className="finance-card-header between"><CardHeader icon={ListChecks} title={selectedYear ? `ماه‌های مالی ${selectedYear.title}` : 'ماه‌های مالی'} bare />{selectedYear && <div className="actions-cell"><button className="mini-btn" disabled={busy || selectedYear.is_closed} onClick={() => onCloseYear(selectedYear.id)}>بستن کل سال</button><button className="mini-btn primary-soft" disabled={busy || !selectedYear.is_closed} onClick={() => onReopenYear(selectedYear.id)}>بازگشایی کل سال</button></div>}</div>
      {selectedYear && <div className="loan-kpis fiscal-kpis"><Info label="شروع سال" value={formatDate(selectedYear.start_date, lang)} /><Info label="پایان سال" value={formatDate(selectedYear.end_date, lang)} /><Info label="ماه‌های بسته" value={`${formatNumber(closedMonths, lang)} از ${formatNumber(periods.length, lang)}`} highlight={closedMonths > 0} /><Info label="وضعیت سال" value={selectedYear.is_closed ? 'بسته' : 'باز'} highlight={selectedYear.is_closed} /></div>}
      {periods.length === 0 ? <Empty t={{ noData: selectedYear ? 'ماه‌های این سال مالی هنوز ساخته نشده‌اند.' : 'ابتدا یک سال مالی اضافه کنید.' }} /> : <div className="table-scroll limited-list tall"><table className="finance-table fiscal-periods-table"><thead><tr><th>شماره</th><th>ماه</th><th>شروع</th><th>پایان</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>{periods.map((p) => <tr key={p.id}><td>{formatNumber(p.period_no, lang)}</td><td>{p.title_fa}</td><td>{formatDate(p.start_date, lang)}</td><td>{formatDate(p.end_date, lang)}</td><td><FiscalStatus closed={p.is_closed} /></td><td className="actions-cell"><button className="mini-btn" disabled={busy || p.is_closed || selectedYear?.is_closed} title={selectedYear?.is_closed ? 'برای تغییر ماه، ابتدا سال مالی را بازگشایی کنید.' : ''} onClick={() => onClosePeriod(p.id)}>بستن ماه</button><button className="mini-btn" disabled={busy || !p.is_closed || selectedYear?.is_closed} title={selectedYear?.is_closed ? 'برای تغییر ماه، ابتدا سال مالی را بازگشایی کنید.' : ''} onClick={() => onReopenPeriod(p.id)}>بازگشایی ماه</button></td></tr>)}</tbody></table></div>}
    </section>
    <BalanceSheetPanel fiscalYear={selectedYear} lang={lang} />
  </div>;
}

function fiscalYearFromIso(value) {
  if (!value) return null;
  const d = new Date(`${String(value).slice(0, 10)}T00:00:00`);
  if (Number.isNaN(d.getTime())) return null;
  return gregorianToJalali(d.getFullYear(), d.getMonth() + 1, d.getDate()).jy;
}
function suggestedFiscalYear(fiscalYears = []) {
  const years = fiscalYears.map((y) => fiscalYearFromIso(y.start_date)).filter(Boolean);
  if (years.length) return Math.max(...years) + 1;
  const now = new Date();
  return gregorianToJalali(now.getFullYear(), now.getMonth() + 1, now.getDate()).jy;
}
function FiscalYearModal({ fiscalYears = [], busy, onClose, onSubmit }) {
  const suggested = suggestedFiscalYear(fiscalYears);
  const [form, setForm] = useState({ jalaliYear: String(suggested), title: `سال مالی ${toPersianDigits(suggested)}` });
  function changeYear(raw) {
    const prevDefault = `سال مالی ${toPersianDigits(form.jalaliYear || '')}`;
    const next = toEnglishDigits(raw).replace(/\D/g, '').slice(0, 4);
    setForm({ ...form, jalaliYear: next, title: !form.title || form.title === prevDefault ? `سال مالی ${toPersianDigits(next)}` : form.title });
  }
  const yearNumber = Number(toEnglishDigits(form.jalaliYear));
  const invalid = !Number.isInteger(yearNumber) || yearNumber < 1300 || yearNumber > 1600;
  return <FinanceModal title="افزودن سال مالی" onClose={onClose}>
    <div className="finance-form-grid fiscal-year-form">
      <label className="finance-field"><span>سال شمسی</span><input dir="ltr" inputMode="numeric" value={toPersianDigits(form.jalaliYear)} onChange={(e)=>changeYear(e.target.value)} placeholder="۱۴۰۶" /></label>
      <label className="finance-field"><span>عنوان سال مالی</span><input value={form.title} onChange={(e)=>setForm({...form,title:e.target.value})} placeholder="سال مالی ۱۴۰۶" /></label>
      <div className="finance-note warning full">با ثبت این فرم، سال مالی از ۱ فروردین همین سال تا پایان اسفند ساخته می‌شود و ۱۲ ماه فروردین تا اسفند به‌صورت خودکار در جدول ماه‌ها اضافه می‌شود.</div>
    </div>
    <div className="finance-form-actions"><button type="button" onClick={onClose}>انصراف</button><button type="button" disabled={busy || invalid || !form.title.trim()} onClick={() => onSubmit?.({ jalaliYear: yearNumber, title: form.title })}>ثبت سال و ایجاد ۱۲ ماه</button></div>
  </FinanceModal>;
}

function BalanceSheetPanel({ fiscalYear, lang }) {
  ...
  async function loadReport() {
    ...
    setError('');
    try {
      const next = await fetchFinanceBalanceSheet({ fiscalYear });
      setReport(next);
    } catch (e) {
      setError(getFriendlyErrorMessage(e, 'خطا در تهیه ترازنامه'));
    } finally {
      setLoading(false);
    }
  }
  function exportExcel() {
    if (!report) return;
    exportBalanceSheetXlsx(report);
  }
  if (!fiscalYear) {
    return <section className="finance-card balance-sheet-card"><CardHeader icon={BookOpen} title="گزارش ترازنامه" /><Empty t={{ noData: 'برای تهیه ترازنامه ابتدا یک سال مالی انتخاب یا اضافه کنید.' }} /></section>;
  }
  return <section className="finance-card balance-sheet-card">
    <div className="finance-card-header between"><CardHeader icon={BookOpen} title={`گزارش ترازنامه ${fiscalYear.title || ''}`} bare /><div className="actions-cell"><button className="mini-btn primary-soft" disabled={loading} onClick={loadReport}>{loading ? 'در حال تهیه...' : 'ساخت / به‌روزرسانی ترازنامه'}</button><button className="mini-btn" disabled={!report} onClick={() => printBalanceSheet(report, lang)}>خروجی PDF</button><button className="mini-btn" disabled={!report} onClick={exportExcel}>خروجی Excel</button></div></div>
    <p className="finance-note">ترازنامه از اسناد حسابداری ثبت‌شده و قطعی‌شده ساخته می‌شود و ساختار اصلی آن براساس دارایی‌ها، بدهی‌ها و حقوق مالکانه است. برای گزارش پایان سال، سال مالی را انتخاب کن و دکمه ساخت ترازنامه را بزن.</p>
    <div className="balance-sheet-guidance">
      <b>این گزارش چگونه پر می‌شود؟</b>
      <ul>
        <li>هر سندی که در سیستم «سند حسابداری قطعی» داشته باشد، خودکار وارد ترازنامه می‌شود.</li>
        <li>فاکتور فروش، خرید و سند هزینه بعد از زدن دکمه «ثبت سند حسابداری» وارد گزارش می‌شوند.</li>
        <li>دریافت/پرداخت، هزینه/درآمد، پرداخت حقوق و تسویه چک/اقساط وقتی سند حسابداری بسازند خودکار وارد می‌شوند.</li>
        <li>مواردی مثل مانده اول دوره، سرمایه اولیه، دارایی‌های قدیمی، موجودی ابتدای سال، استهلاک، مالیات و اصلاحات پایان سال باید با سند حسابداری/افتتاحیه ثبت شوند تا در ترازنامه بیایند.</li>
      </ul>
    </div>
    {error && <div className="accounting-message error">{error}</div>}
    {!report ? <div className="finance-empty balance-sheet-empty">هنوز ترازنامه ساخته نشده است.</div> : <>
      <div className="loan-kpis balance-sheet-kpis"><Info label="تاریخ گزارش" value={formatDate(report.asOfDate, lang)} /><Info label="جمع دارایی‌ها" value={formatRial(report.totals.assets)} highlight /><Info label="بدهی + حقوق مالکانه" value={formatRial(report.totals.liabilitiesAndEquity)} highlight /><Info label="اختلاف تراز" value={formatRial(report.totals.difference)} highlight={!report.totals.isBalanced} /></div>
      <div className={report.totals.isBalanced ? 'finance-note success' : 'finance-note warning'}>{report.totals.isBalanced ? 'ترازنامه از نظر جمع کل تراز است.' : 'اختلاف تراز وجود دارد؛ احتمالاً بعضی اسناد حسابداری هنوز ثبت قطعی نشده‌اند یا افتتاحیه/اختتامیه کامل نیست.'} تعداد اسناد منبع: {formatNumber(report.sourceEntryCount, lang)} · تعداد ردیف سند: {formatNumber(report.sourceLineCount, lang)}</div>
      <div className="balance-sheet-grid">
        <BalanceSheetSection title="دارایی‌ها" rows={report.sections.assets} total={report.totals.assets} lang={lang} />
        <BalanceSheetSection title="بدهی‌ها" rows={report.sections.liabilities} total={report.totals.liabilities} lang={lang} />
        <BalanceSheetSection title="حقوق مالکانه" rows={report.sections.equity} total={report.totals.equity} lang={lang} />
      </div>
      <div className="balance-sheet-equation"><span>دارایی‌ها</span><b>{formatRial(report.totals.assets)}</b><span>=</span><span>بدهی‌ها + حقوق مالکانه</span><b>{formatRial(report.totals.liabilitiesAndEquity)}</b></div>
    </>}
  </section>;
}

function BalanceSheetSection({ title, rows = [], total = 0 }) {
  return <div className="balance-sheet-section"><h3>{title}</h3>{rows.length === 0 ? <div className="finance-empty small">ردیفی ثبت نشده است.</div> : <table className="finance-table compact"><thead><tr><th>کد</th><th>حساب</th><th>مبلغ</th></tr></thead><tbody>{rows.map((row) => <tr key={row.account_id || row.account_code} className={row.is_result_row ? 'soft-row' : ''}><td dir="ltr">{row.account_code}</td><td>{row.account_name_fa}</td><td className={Number(row.balance) < 0 ? 'negative' : 'positive'}>{formatRial(row.balance)}</td></tr>)}</tbody><tfoot><tr><td colSpan={2}>جمع {title}</td><td>{formatRial(total)}</td></tr></tfoot></table>}</div>;
}

function exportBalanceSheetXlsx(report) {
  const rows = [
    [`ترازنامه ${report.fiscalYear?.title || ''}`],
    [`تاریخ گزارش: ${formatDate(report.asOfDate, 'fa')}`],
    [],
    ['بخش', 'کد حساب', 'عنوان حساب', 'مبلغ ریال'],
    ...balanceSheetExcelRows(report),
  ];
  const ws = XLSX.utils.aoa_to_sheet(rows);
  ws['!cols'] = [{ wch: 24 }, { wch: 14 }, { wch: 34 }, { wch: 20 }];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Balance Sheet');
  XLSX.writeFile(wb, `balance-sheet-${safeFilePart(report.fiscalYear?.title || 'fiscal-year')}.xlsx`);
}
function balanceSheetExcelRows(report) {
  const rows = [];
  const addSection = (title, list, total) => {
    rows.push([title, '', '', '']);
    list.forEach((row) => rows.push([title, row.account_code, row.account_name_fa, Math.round(Number(row.balance || 0))]));
    rows.push([`جمع ${title}`, '', '', Math.round(Number(total || 0))]);
  };
  addSection('دارایی‌ها', report.sections.assets, report.totals.assets);
  addSection('بدهی‌ها', report.sections.liabilities, report.totals.liabilities);
  addSection('حقوق مالکانه', report.sections.equity, report.totals.equity);
  rows.push(['جمع بدهی‌ها و حقوق مالکانه', '', '', Math.round(Number(report.totals.liabilitiesAndEquity || 0))]);
  rows.push(['اختلاف تراز', '', '', Math.round(Number(report.totals.difference || 0))]);
  return rows;
}
function balanceRowsHtml(title, rows, total) {
  const bodyRows = rows.length ? rows.map((row) => `<tr class="${row.is_result_row ? 'soft-row' : ''}"><td>${htmlSafe(title)}</td><td dir="ltr">${htmlSafe(row.account_code)}</td><td>${htmlSafe(row.account_name_fa)}</td><td class="money">${formatRial(row.balance)}</td></tr>`).join('') : `<tr><td>${htmlSafe(title)}</td><td colspan="3">ردیفی ثبت نشده است.</td></tr>`;
  return `<tr class="soft-row"><td colspan="4"><b>${htmlSafe(title)}</b></td></tr>${bodyRows}<tr><td colspan="3"><b>جمع ${htmlSafe(title)}</b></td><td class="money"><b>${formatRial(total)}</b></td></tr>`;
}
function printBalanceSheet(report, lang = 'fa') {
  if (!report) return;
  const rows = [
    balanceRowsHtml('دارایی‌ها', report.sections.assets, report.totals.assets),
    balanceRowsHtml('بدهی‌ها', report.sections.liabilities, report.totals.liabilities),
    balanceRowsHtml('حقوق مالکانه', report.sections.equity, report.totals.equity),
    `<tr class="soft-row"><td colspan="3"><b>جمع بدهی‌ها و حقوق مالکانه</b></td><td class="money"><b>${formatRial(report.totals.liabilitiesAndEquity)}</b></td></tr>`,
    `<tr><td colspan="3"><b>اختلاف تراز</b></td><td class="money"><b>${formatRial(report.totals.difference)}</b></td></tr>`,
  ].join('');
  const body = `<div class="statement-title"><h1>ترازنامه حسابداری</h1><h2>${htmlSafe(report.fiscalYear?.title || '')} · تاریخ گزارش: ${formatDate(report.asOfDate, lang)}</h2></div><section class="statement-summary"><div><span>جمع دارایی‌ها</span><strong class="money">${formatRial(report.totals.assets)}</strong></div><div><span>جمع بدهی‌ها</span><strong class="money">${formatRial(report.totals.liabilities)}</strong></div><div><span>جمع حقوق مالکانه</span><strong class="money">${formatRial(report.totals.equity)}</strong></div><div><span>اختلاف تراز</span><strong class="money">${formatRial(report.totals.difference)}</strong></div></section><div class="section-label">اقلام ترازنامه</div><table class="official-table compact"><thead><tr><th style="width:22%">بخش</th><th style="width:14%">کد حساب</th><th>عنوان حساب</th><th style="width:24%">مبلغ</th></tr></thead><tbody>${rows}</tbody></table><div class="notes-box"><b>یادداشت گزارش:</b> این ترازنامه از اسناد حسابداری قطعی‌شده تا تاریخ گزارش تهیه شده است. سود یا زیان سال جاری در بخش حقوق مالکانه نمایش داده شده تا معادله ترازنامه کنترل شود.<br><b>وضعیت تراز:</b> ${report.totals.isBalanced ? 'تراز است.' : 'دارای اختلاف تراز است و نیاز به بررسی اسناد، افتتاحیه یا اختتامیه دارد.'}</div><section class="signatures"><span>تهیه‌کننده</span><span>مدیر مالی</span><span>مدیر عامل</span></section>`;
  openOfficialFinancePrint({ title: `ترازنامه ${report.fiscalYear?.title || ''}`, reportLabel: 'ترازنامه حسابداری', subtitle: `تا تاریخ ${formatDate(report.asOfDate, lang)}`, orientation: 'portrait', layout: 'statement', meta: { date: formatDate(report.asOfDate, lang) }, body });
}
function safeFilePart(value) { return String(value || 'report').replace(/[\\/:*?"<>|\s]+/g, '_'); }


function SettingsSection({ numbering, ioDocuments, lang, busy, onUpdateNumbering, onAddIo, currencyDisplay = 'toman', onCurrencyDisplayChange }) {
